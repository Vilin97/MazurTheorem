...
valuation among integral admissible changes.  This file extracts the two forms used at the start
of a Tate-algorithm argument:

* integral `j` gives `v(c₄)³ ≤ v(Δ)`; and
* a pure change of scale by an element of valuation below one cannot keep all five weighted
...
theorem not_isIntegral_scale_of_isMinimal
  ...
  have huPos : 0 < valuation K (maximalIdeal R) (u : K) :=
    (valuation K (maximalIdeal R)).pos_iff.mpr u.ne_zero
  have huInv : 1 < (valuation K (maximalIdeal R) (u : K))⁻¹ :=
    one_lt_inv_iff₀.mpr ⟨huPos, hu⟩
  have huInvPow : 1 < (valuation K (maximalIdeal R) (u : K))⁻¹ ^ 12 :=
