...
theorem canonicalDualTripleMap_assoc_tmul_apply_tmul_tmul
  ...
  | zero =>
      ...
      rw [(TensorProduct.assoc R
        (DualCoordinates R G.coordinates)
        (DualCoordinates R G.coordinates)
        (DualCoordinates R G.coordinates)).map_zero,
        (canonicalDualTripleMap G).map_zero,
        (canonicalDualTensorMap G).map_zero]
      change (0 : R) = 0 * h.ofConv c
...
theorem canonicalDualTripleMap_tmul_apply_tmul_tmul
  ...
  | zero =>
      ...
      rw [(canonicalDualTripleMap G).map_zero,
        (canonicalDualTensorMap G).map_zero]
      change (0 : R) = f.ofConv a * 0
...
/-- Finite-free commutative Hopf coordinates carry an actual Cartier-dual Hopf algebra. -/
...
/-- The linear Cartier-dual coordinates of the bundled constant Hopf presentation. -/
abbrev ConstantDualCoordinates :=
  DualCoordinates R (FiniteFlatCommGroupScheme.constant R M).coordinates

/-- Forget the universe-preserving wrapper on the bundled constant coordinates. -/
def bundledConstantCoordinatesLinearEquiv :
    (FiniteFlatCommGroupScheme.constant R M).coordinates ≃ₗ[R] (M → R) where
  toFun f := f.down
  invFun f := ULift.up f
...
/-- The delta-function basis, constructed with the exact module instance stored in the bundled
constant Hopf presentation. -/
def bundledConstantCoordinatesBasis :
    Module.Basis M R (FiniteFlatCommGroupScheme.constant R M).coordinates :=
  (Pi.basisFun R M).map (bundledConstantCoordinatesLinearEquiv R M).symm

/-- Evaluation at `m` as a linear form on the exact bundled constant coordinates. -/
def bundledConstantEvaluationLinearMap (m : M) :
    (FiniteFlatCommGroupScheme.constant R M).coordinates →ₗ[R] R where
  toFun f := f.down m
  map_add' _ _ := rfl
  map_smul' _ _ := rfl
...
def dualConstantEvaluation (m : M) :
    ConstantDualCoordinates R M :=
  WithConv.toConv (bundledConstantEvaluationLinearMap R M m)
...
def dualConstantBasis :
    Module.Basis M R (ConstantDualCoordinates R M) := by
  classical
  exact (bundledConstantCoordinatesBasis R M).dualBasis.map
    (WithConv.linearEquiv R
      (Module.Dual R (FiniteFlatCommGroupScheme.constant R M).coordinates)).symm
...
theorem dualConstantBasis_apply (m : M) :
  ...
  apply LinearMap.ext
  intro f
  change ((bundledConstantCoordinatesBasis R M).dualBasis m) f =
    bundledConstantEvaluationLinearMap R M m f
  rw [Module.Basis.dualBasis_apply]
  change ((Pi.basisFun R M).repr
      (bundledConstantCoordinatesLinearEquiv R M f)) m = f.down m
  rw [Pi.basisFun_repr]
  rfl
...
theorem dualConstantEvaluation_mul (m n : M) :
  ...
  change
    WithConv.toConv (ConstantCoordinates.evaluation (R := R) m).toLinearMap *
        WithConv.toConv (ConstantCoordinates.evaluation (R := R) n).toLinearMap =
      WithConv.toConv
        (ConstantCoordinates.evaluation (R := R) (m * n)).toLinearMap
  exact h'
...
def dualConstantLinearEquiv :
    ConstantDualCoordinates R M ≃ₗ[R] MonoidAlgebra R M :=
...
theorem dualConstantLinearEquiv_map_mul
    (x y : ConstantDualCoordinates R M) :
...
def dualConstantBialgEquiv :
    ConstantDualCoordinates R M ≃ₐc[R] MonoidAlgebra R M := by
  letI hCoalgebra : Coalgebra R (ConstantDualCoordinates R M) :=
    instCoalgebraDualCoordinates (FiniteFlatCommGroupScheme.constant R M)
  letI hBialgebra : Bialgebra R (ConstantDualCoordinates R M) :=
  ...
  let c : ConstantDualCoordinates R M ≃ₗc[R] MonoidAlgebra R M :=
    { e.toLinearEquiv with
      counit_comp := by
        ...
        change Coalgebra.counit (R := R)
            (e (dualConstantEvaluation R M m)) =
          dualCounit (FiniteFlatCommGroupScheme.constant R M)
            (dualConstantEvaluation R M m)
        rw [dualConstantAlgEquiv_evaluation,
          MonoidAlgebra.counit_single,
          dualCounit_dualConstantEvaluation]
        exact Bialgebra.counit_one
      map_comp_comul := by
        apply b.ext
        intro m
        ...
        change TensorProduct.map e.toLinearMap e.toLinearMap
            (dualComul (FiniteFlatCommGroupScheme.constant R M)
              (dualConstantEvaluation R M m)) =
          Coalgebra.comul (R := R) (e (dualConstantEvaluation R M m))
