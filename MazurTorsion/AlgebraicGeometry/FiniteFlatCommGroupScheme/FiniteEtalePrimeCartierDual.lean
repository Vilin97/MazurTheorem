    ...
    { dualLinearEquivOfBialgEquiv e with
      ...
      map_comp_comul := by
        ...
        rw [canonicalDualTensorMap_map_dualLinearEquivOfBialgEquiv,
          ...
          canonicalDualTensorMap_dualComul_apply_tmul]
        change f.ofConv (e.symm a * e.symm b) =
          f.ofConv (e.symm (a * b))
        exact congrArg f.ofConv (e.symm.map_mul a b).symm }
...
def coordinateBialgEquivOfIso (e : G ≅ H) :
    G.coordinates ≃ₐc[R] H.coordinates :=
  CommHopfAlgCat.isoEquivBialgEquiv
    (((AlgebraicGeometry.cocommutativeHopfProperty R).ι.mapIso
      (((affineFiniteFreeProperty R).ι.mapIso e).unop)).symm)
