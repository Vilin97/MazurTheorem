...
/-- A unit has trivial geometric Kummer boundary exactly when it is an actual `n`th power.
The reverse implication uses subcanonical fppf descent to glue a global `G_m` lift from a
trivialization of the root-cover cocycle. -/
theorem unitBoundaryHom_eq_one_iff [Nontrivial R]
    (n : ℕ) (hn : n ≠ 0) (u : Rˣ) :
    unitBoundaryHom R n hn u = 1 ↔ ∃ v : Rˣ, v ^ n = u := by
  constructor
  · intro hu
    change boundaryHom R n hn
      ((AffineCommGroupScheme.multiplicativeBasePointMulEquiv R).symm u) = 1 at hu
    obtain ⟨x, hx⟩ :=
      (locallyLiftable R n hn).exists_source_of_boundaryHom_eq_one
        ((AffineCommGroupScheme.multiplicativeBasePointMulEquiv R).symm u) hu
    refine ⟨AffineCommGroupScheme.multiplicativeBasePointMulEquiv R x, ?_⟩
    have hx' := congrArg
      (AffineCommGroupScheme.multiplicativeBasePointMulEquiv R) hx
    simpa only [CommGroupScheme.mapPoint_powerEnd, map_pow,
      MulEquiv.apply_symm_apply] using hx'
  · rintro ⟨v, rfl⟩
    exact unitBoundaryHom_pow R n hn v
...
/-- The quotient Kummer boundary is injective: its kernel is exactly the subgroup of actual
`n`th powers, by the checked fppf descent theorem above. -/
theorem unitKummerBoundaryHom_injective [Nontrivial R]
    (n : ℕ) (hn : n ≠ 0) :
    Function.Injective (unitKummerBoundaryHom R n hn) := by
  apply (MonoidHom.ker_eq_bot_iff (unitKummerBoundaryHom R n hn)).mp
  apply le_antisymm
  · intro z hz
    obtain ⟨u, rfl⟩ := QuotientGroup.mk'_surjective
      (powMonoidHom n : Rˣ →* Rˣ).range z
    have hu : unitBoundaryHom R n hn u = 1 := by
      rw [← unitKummerBoundaryHom_mk R n hn]
      exact MonoidHom.mem_ker.mp hz
    obtain ⟨v, hv⟩ := (unitBoundaryHom_eq_one_iff R n hn u).mp hu
    change QuotientGroup.mk' (powMonoidHom n : Rˣ →* Rˣ).range u = 1
    apply (QuotientGroup.eq_one_iff u).mpr
    exact MonoidHom.mem_range.mpr ⟨v, hv⟩
  · exact bot_le

/-- Units modulo powers are therefore identified with the actual subgroup of geometric fppf
`H¹` generated by Kummer boundaries.  This carrier-preserving equivalence is the numerical
handoff used by finite-flat cohomology consumers. -/
noncomputable def unitKummerBoundaryMulEquivRange [Nontrivial R]
    (n : ℕ) (hn : n ≠ 0) :
    FiniteFlatCommGroupScheme.UnitKummerClasses R n ≃*
      (unitKummerBoundaryHom R n hn).range :=
  MulEquiv.ofBijective (unitKummerBoundaryHom R n hn).rangeRestrict
    ⟨fun _ _ hxy ↦ unitKummerBoundaryHom_injective R n hn
