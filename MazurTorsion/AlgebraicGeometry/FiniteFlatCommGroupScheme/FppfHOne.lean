...
For actual fppf covers of a scheme, `FppfHOne` is the filtered-colimit quotient of all cover-level
classes by common refinement.  We use the explicit quotient presentation of a colimit of types,
rather than asking Lean for a small category of all covers.  The latter would require a smallness
instance which the large indexed-cover category does not possess.  The relation is generated only
by genuine `Scheme.Cover.Hom` refinements, so no unrelated group is being renamed as fppf
cohomology.
-/
...
structure FppfHOneRepresentative (X : Scheme.{u}) (G : Scheme.{u}ᵒᵖ ⥤ GrpCat.{w}) where
  /-- The fppf covering family on which the cocycle is defined. -/
  cover : Cover.{v} fppfPrecoverage X
  /-- Its cover-level nonabelian cohomology class. -/
  value : PresheafOfGroups.H1 G cover.X

namespace FppfHOneRepresentative

variable {X : Scheme.{u}} {G : Scheme.{u}ᵒᵖ ⥤ GrpCat.{w}}
...
def IsEquivalent (a b : FppfHOneRepresentative.{w, v, u} X G) : Prop :=
  ∃ (𝒲 : Cover.{v} fppfPrecoverage X)
    (ra : 𝒲.Hom a.cover) (rb : 𝒲.Hom b.cover),
      PreZeroHypercover.Hom.pullbackHOne G ra a.value =
        PreZeroHypercover.Hom.pullbackHOne G rb b.value
...
lemma isEquivalent_trans {a b c : FppfHOneRepresentative.{w, v, u} X G}
  ...
  rw [PreZeroHypercover.Hom.pullbackHOne_comp, PreZeroHypercover.Hom.pullbackHOne_comp]
  calc
    PreZeroHypercover.Hom.pullbackHOne G q𝒲
        (PreZeroHypercover.Hom.pullbackHOne G ra a.value) =
        PreZeroHypercover.Hom.pullbackHOne G q𝒲
          (PreZeroHypercover.Hom.pullbackHOne G rb b.value) :=
      congrArg (PreZeroHypercover.Hom.pullbackHOne G q𝒲) hab
    _ = PreZeroHypercover.Hom.pullbackHOne G q𝒵
        (PreZeroHypercover.Hom.pullbackHOne G sb b.value) := by
      rw [← PreZeroHypercover.Hom.pullbackHOne_comp,
        ← PreZeroHypercover.Hom.pullbackHOne_comp]
      exact PreZeroHypercover.Hom.pullbackHOne_eq G _ _ b.value
    _ = PreZeroHypercover.Hom.pullbackHOne G q𝒵
        (PreZeroHypercover.Hom.pullbackHOne G sc c.value) :=
      congrArg (PreZeroHypercover.Hom.pullbackHOne G q𝒵) hbc
...
of cover-level classes by genuine common refinement.  The cover index universe is explicit. -/
def FppfHOne (X : Scheme.{u}) (G : Scheme.{u}ᵒᵖ ⥤ GrpCat.{w}) : Type _ :=
...
def mk (𝒰 : Cover.{v} fppfPrecoverage X) (x : PresheafOfGroups.H1 G 𝒰.X) :
    FppfHOne.{w, v, u} X G :=
  Quot.mk _ ⟨𝒰, x⟩
...
theorem mk_one (𝒰 : Cover.{v} fppfPrecoverage X) :
    mk 𝒰 (1 : PresheafOfGroups.H1 G 𝒰.X) = (1 : FppfHOne.{w, v, u} X G) := by
  apply Quot.sound
  let e : 𝒰.Hom 𝒰 := PreZeroHypercover.Hom.id 𝒰.toPreZeroHypercover
  exact ⟨𝒰, e, toIdentityCover 𝒰, by
    rw [PreZeroHypercover.Hom.pullbackHOne_one,
      PreZeroHypercover.Hom.pullbackHOne_one]⟩
...
theorem class_pullback (𝒰 𝒱 : Cover.{v} fppfPrecoverage X) (r : 𝒱.Hom 𝒰)
    (x : PresheafOfGroups.H1 G 𝒰.X) :
    mk 𝒱 (PreZeroHypercover.Hom.pullbackHOne G r x) = mk 𝒰 x := by
  apply Quot.sound
  let e : 𝒱.Hom 𝒱 := PreZeroHypercover.Hom.id 𝒱.toPreZeroHypercover
  refine ⟨𝒱, e, r, ?_⟩
  dsimp only [e]
  exact PreZeroHypercover.Hom.pullbackHOne_id G _
...
theorem class_eq_iff (𝒰 𝒱 : Cover.{v} fppfPrecoverage X)
    (x : PresheafOfGroups.H1 G 𝒰.X) (y : PresheafOfGroups.H1 G 𝒱.X) :
    mk 𝒰 x = mk 𝒱 y ↔
      ∃ (𝒲 : Cover.{v} fppfPrecoverage X)
        (r : 𝒲.Hom 𝒰) (s : 𝒲.Hom 𝒱),
          PreZeroHypercover.Hom.pullbackHOne G r x =
            PreZeroHypercover.Hom.pullbackHOne G s y :=
...
def lift {Y : Sort*}
    (f : ∀ (𝒰 : Cover.{v} fppfPrecoverage X), PresheafOfGroups.H1 G 𝒰.X → Y)
    (compatible : ∀ (𝒰 𝒱 : Cover.{v} fppfPrecoverage X) (r : 𝒱.Hom 𝒰)
      (x : PresheafOfGroups.H1 G 𝒰.X),
      f 𝒱 (PreZeroHypercover.Hom.pullbackHOne G r x) = f 𝒰 x) :
  ...
  Quot.lift
    ...
    (by
      ...
      calc
        f 𝒰 x = f 𝒲 (PreZeroHypercover.Hom.pullbackHOne G r x) :=
          (compatible 𝒰 𝒲 r x).symm
        _ = f 𝒲 (PreZeroHypercover.Hom.pullbackHOne G s y) := congrArg (f 𝒲) h
...
theorem lift_mk {Y : Sort*}
    ...
    (compatible : ∀ (𝒰 𝒱 : Cover.{v} fppfPrecoverage X) (r : 𝒱.Hom 𝒰)
      (x : PresheafOfGroups.H1 G 𝒰.X),
      f 𝒱 (PreZeroHypercover.Hom.pullbackHOne G r x) = f 𝒰 x)
    (𝒰 : Cover.{v} fppfPrecoverage X) (x : PresheafOfGroups.H1 G 𝒰.X) :
...
theorem funext {Y : Sort*} {f g : FppfHOne.{w, v, u} X G → Y}
    (h : ∀ (𝒰 : Cover.{v} fppfPrecoverage X) (x : PresheafOfGroups.H1 G 𝒰.X),
      f (mk 𝒰 x) = g (mk 𝒰 x)) :
    f = g := by
  funext z
