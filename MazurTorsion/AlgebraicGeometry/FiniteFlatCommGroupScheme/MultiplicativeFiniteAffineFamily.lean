...
/-!
# Finite affine families and singleton product covers

For a finite affine fppf family `Spec (B i) → Spec A`, this file constructs the singleton
fppf cover `Spec (∏ i, B i) → Spec A`.  It identifies the disjoint union of the original
family with the spectrum of the product algebra, constructs the genuine cover refinement from
the original family to the singleton product, and checks its component maps are the algebra
projections.

A represented `G_m` cocycle on the singleton product therefore pulls back to the original
family.  The resulting cover cocycle has an explicit evaluator, and Mathlib's global
`FppfHOne.class_pullback` shows that the two cover-level representatives define the same global
class.

The direction is essential: the original family refines the singleton product family, so this
API pulls singleton cocycles to component cocycles.  It does not turn an arbitrary cocycle on
the original family into a singleton cocycle.  Such a reverse passage requires a compatible
gauge or an effective sheaf-gluing theorem and is not claimed here.
-/
