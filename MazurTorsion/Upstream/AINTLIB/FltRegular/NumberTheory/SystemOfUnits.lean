...
public import MazurTorsion.Upstream.AINTLIB.FltRegular.NumberTheory.CyclotomicRing
...
lemma ex_not_mem [Module.Free ℤ G] {R : ℕ} (S : systemOfUnits p G R) (hR : R < s) :
  ...
  have hfinrank :
      finrank ℤ ((Submodule.span A (Set.range S.units)).restrictScalars ℤ) =
        finrank ℤ (Submodule.span A (Set.range S.units)) :=
    ((Submodule.restrictScalarsEquiv ℤ A G
      (Submodule.span A (Set.range S.units))).restrictScalars ℤ).finrank_eq
