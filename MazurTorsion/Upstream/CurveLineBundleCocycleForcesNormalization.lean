...
invertible. The result is pseudofunctor-generic so concrete descent APIs can use it without
substituting large geometric transition terms into the proof.
-/
...
private theorem diagonal_pullHom_idempotent_of_cocycle
...
private theorem pullHom_isIso_of_isIso
    {C : Type u} [Category.{v} C]
    {F : Pseudofunctor (LocallyDiscrete Cᵒᵖ) Cat}
    {X₁ X₂ Y Y' : C}
