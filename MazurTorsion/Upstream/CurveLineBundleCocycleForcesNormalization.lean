...
/-- An isomorphism-valued overlap cocycle supplies normalized chosen-overlap descent data.
This constructor stays at the pseudofunctor level so concrete consumers do not need to convert
their transition family through a second, definitionally equal pullback API. -/
noncomputable irreducible_def descentDataPrimeOfIsoCocycle
    {C : Type u} [Category.{v} C]
    {F : Pseudofunctor (LocallyDiscrete Cᵒᵖ) Cat}
    {ι : Type*} {S : C} {Y : ι → C} {g : ∀ i, Y i ⟶ S}
    {sq : ∀ i j, ChosenPullback (g i) (g j)}
  ...
  pullHom'_hom_self := normalization_of_iso_cocycle overlapIso hcocycle
  pullHom'_hom_comp := hcocycle

/-- The generic cocycle constructor retains its specified local objects. -/
theorem descentDataPrimeOfIsoCocycle_obj
    {C : Type u} [Category.{v} C]
    {F : Pseudofunctor (LocallyDiscrete Cᵒᵖ) Cat}
    {ι : Type*} {S : C} {Y : ι → C} {g : ∀ i, Y i ⟶ S}
    {sq : ∀ i j, ChosenPullback (g i) (g j)}
    ...
    (hcocycle : ∀ i j k,
      Pseudofunctor.DescentData'.pullHom' (fun i j ↦ (overlapIso i j).hom)
        ...
        Pseudofunctor.DescentData'.pullHom' (fun i j ↦ (overlapIso i j).hom)
          (sq₃ i j k).p (sq₃ i j k).p₁ (sq₃ i j k).p₃)
    (i : ι) :
    (descentDataPrimeOfIsoCocycle obj overlapIso hcocycle).obj i = obj i := by
  rw [descentDataPrimeOfIsoCocycle_def]
