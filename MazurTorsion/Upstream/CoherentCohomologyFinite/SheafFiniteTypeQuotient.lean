...
Finite local generators descend along an epimorphism.  In particular,
finite-type sheaves of modules are closed under cokernels.
-/
...
theorem isFiniteType_of_epi
  ...
  have hEpi (i : P.I) : Epi (p.over (P.X i)) := by
    let adj :=
      overPushforwardOverAdj (R := R) (P.X i)
    letI :
