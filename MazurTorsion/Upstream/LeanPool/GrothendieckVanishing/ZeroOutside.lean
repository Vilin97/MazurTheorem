...
private lemma zeroOutside_map_heq {W Y : (Opens X)ᵒᵖ} (i : W ⟶ Y)
    (hW : unop W ≤ U) :
    (zeroOutside U F).map i ≍ F.map i := by
...
def zeroOutsideTopIso : zeroOutside (⊤ : Opens X) F ≅ F :=
  NatIso.ofComponents
    ...
    (fun {W Y} i ↦ by
      ...
      have hmap : (zeroOutside (⊤ : Opens X) F).map i ≍ F.map i :=
...
def zeroOutsideOpenHom : zeroOutside V F ⟶ zeroOutside U F where
  ...
  naturality {W Y} i := by
    ...
    · have hYV : unop Y ≤ V := le_trans (leOfHom i.unop) hWV
      ...
      have hmapV : (zeroOutside V F).map i ≍ F.map i :=
        zeroOutside_map_heq i hWV
      have hmapU : (zeroOutside U F).map i ≍ F.map i :=
...
def sHom {F : Presheaf AddCommGrpCat.{u} X} (s : F.obj (op U)) :
  ...
  naturality {W Y} i := by
    ...
    · have hYU : (unop Y) ≤ U := le_trans (leOfHom i.unop) hWU
      ...
      let mW := AddCommGrpCat.ofHom (uliftZMultiplesHom (F.obj W) sW)
      let mY := AddCommGrpCat.ofHom (uliftZMultiplesHom (F.obj Y) sY)
      ...
      have hObjY : (zeroOutside U constZ).obj Y = AddCommGrpCat.of (ULift ℤ) := by
        simp [zeroOutside, hYU, constZ]
      have hzero : (zeroOutside U constZ).map i ≍
          𝟙 (AddCommGrpCat.of (ULift ℤ)) := by
        simpa [constZ] using
      ...
      have happY :
        ...
        eqToHom_comp_heq _ _
      have hleft :
          (zeroOutside U constZ).map i ≫
              (eqToHom hObjY ≫ mY) ≍ mY := by
        have hc := heq_comp hObjW hObjY rfl hzero happY
        simpa using hc
      have hright :
          (eqToHom hObjW ≫ mW) ≫ F.map i ≍ mY := by
        have hc := heq_comp hObjW rfl rfl happW (HEq.rfl : F.map i ≍ F.map i)
...
theorem _root_.stalk_zeroOutsideInt_eq_zsmul_generator
  ...
  have hn' : q = n • qgen := hn
  have hmapn :
      (AddCommGrpCat.Hom.hom (T.map (toSheafify J P))) (n • qgen) =
        n • (AddCommGrpCat.Hom.hom (T.map (toSheafify J P))) qgen :=
    map_zsmul (AddCommGrpCat.Hom.hom (T.map (toSheafify J P))) n qgen
  have hgen :
      (AddCommGrpCat.Hom.hom (T.map (toSheafify J P))) qgen =
  ...
  exact (congrArg (AddCommGrpCat.Hom.hom (T.map (toSheafify J P))) hn').trans
