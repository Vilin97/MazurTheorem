...
import MazurTorsion.Upstream.LeanPool.GrothendieckVanishing.ClosedImmersion
...
noncomputable def sheafCohomologyFunctor (X : TopCat.{u}) (n : ℕ) :
    TopCat.Sheaf AddCommGrpCat.{u} X ⥤ AddCommGrpCat.{u} := by
  change CategoryTheory.Sheaf (Opens.grothendieckTopology X) AddCommGrpCat.{u} ⥤
    AddCommGrpCat.{u}
  exact extFunctorObj ((constantSheaf (Opens.grothendieckTopology X) AddCommGrpCat).obj
...
private theorem sheafH_comp_extClass_naturality {X : TopCat.{u}}
  ...
  change
    Sheaf.H.map φ.τ₁ (n + 1) (Sheaf.H.δ hS₁ n (n + 1) rfl y) =
      Sheaf.H.δ hS₂ n (n + 1) rfl (Sheaf.H.map φ.τ₃ n y)
  exact (Sheaf.H.δ_naturality n (n + 1) rfl hS₁ hS₂ φ y).symm
...
noncomputable def sheafHSuccMap {X : TopCat.{u}}
    ...
    (hS : S.ShortExact)
    (n : ℕ) :=
  AddCommGrpCat.ofHom (Sheaf.H.δ hS n (n + 1) rfl)
...
noncomputable def sheafH0SectionsInv {X : TopCat.{u}}
    (F : TopCat.Sheaf AddCommGrpCat.{u} X) := by
  letI : AddCommGroup (Sheaf.H F 0) :=
    CategoryTheory.Abelian.Ext.instAddCommGroup
  exact AddCommGrpCat.ofHom ((sheafH0EquivSections F).symm.toAddMonoidHom)
...
noncomputable def sheafH1CokernelIsoOfSubsingletonMiddle {X : TopCat.{u}}
    ...
    cokernel (S.g.hom.app (op ⊤)) ≅ (sheafCohomologyFunctor X 1).obj S.X₁ := by
  ...
  have hπH_epi : Epi πH := by
    ...
    have hinv : ConcreteCategory.hom (sheafH0SectionsInv S.X₃)
        (sheafH0EquivSections S.X₃ y) = y := by
      change (sheafH0EquivSections S.X₃).symm (sheafH0EquivSections S.X₃ y) = y
      exact (sheafH0EquivSections S.X₃).symm_apply_apply y
    have htarget : ConcreteCategory.hom (sheafHSuccMap hS 0)
        (ConcreteCategory.hom (sheafH0SectionsInv S.X₃)
          (sheafH0EquivSections S.X₃ y)) = x := by
      rw [hinv]
      exact hy
    simpa [πH, δ] using htarget
  have hπH_mono : Mono πH := by
    ...
    have hzero : z.comp hS.extClass rfl = 0 := by
      have hmap :
          ConcreteCategory.hom (sheafHSuccMap hS 0) z = 0 := by
  ...
  letI : Epi πH := hπH_epi
  letI : Mono πH := hπH_mono
  exact @asIso _ _ _ _ πH (isIso_of_mono_of_epi πH)
