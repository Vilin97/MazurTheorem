...
private def leftRow :=
  (((((leftTailFour.take 5).drop 1).drop 1).drop 1).drop 1)

private def rightRow :=
  (((((rightTailFour.take 5).drop 1).drop 1).drop 1).drop 1)

theorem certificateBlockFourRowFour : leftRow = rightRow := by decide
