...
private def leftRow :=
  ((((leftTailOne.take 5).drop 1).drop 1).drop 1).take 1

private def rightRow :=
  ((((rightTailOne.take 5).drop 1).drop 1).drop 1).take 1

theorem certificateBlockOneRowThree : leftRow = rightRow := by decide
