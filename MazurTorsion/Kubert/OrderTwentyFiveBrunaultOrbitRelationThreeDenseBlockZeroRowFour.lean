...
private def leftRow :=
  (((((leftCertificate.take 5).drop 1).drop 1).drop 1).drop 1)

private def rightRow :=
  (((((rightCertificate.take 5).drop 1).drop 1).drop 1).drop 1)

theorem certificateBlockZeroRowFour : leftRow = rightRow := by decide
