...
private def leftRow :=
  (((((leftTailOne.take 5).drop 1).drop 1).drop 1).drop 1)

private def rightRow :=
  (((((rightTailOne.take 5).drop 1).drop 1).drop 1).drop 1)

theorem certificateBlockOneRowFour : leftRow = rightRow := by decide
