...
private def leftRow :=
  ((((leftTailFour.take 5).drop 1).drop 1).drop 1).take 1

private def rightRow :=
  ((((rightTailFour.take 5).drop 1).drop 1).drop 1).take 1

theorem certificateBlockFourRowThree : leftRow = rightRow := by decide
