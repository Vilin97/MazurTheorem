...
private def leftRow :=
  (((((leftTailTwo.take 5).drop 1).drop 1).drop 1).drop 1)

private def rightRow :=
  (((((rightTailTwo.take 5).drop 1).drop 1).drop 1).drop 1)

theorem certificateBlockTwoRowFour : leftRow = rightRow := by decide
