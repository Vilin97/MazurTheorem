...
private def leftRow :=
  ((((leftTailFive.take 5).drop 1).drop 1).drop 1).take 1

private def rightRow :=
  ((((rightTailFive.take 5).drop 1).drop 1).drop 1).take 1

theorem certificateBlockFiveRowThree : leftRow = rightRow := by decide
