...
private def leftRow :=
  (((((leftTailFive.take 5).drop 1).drop 1).drop 1).drop 1)

private def rightRow :=
  (((((rightTailFive.take 5).drop 1).drop 1).drop 1).drop 1)

theorem certificateBlockFiveRowFour : leftRow = rightRow := by decide
