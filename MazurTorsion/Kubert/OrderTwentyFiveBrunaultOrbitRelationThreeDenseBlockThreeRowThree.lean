...
private def leftRow :=
  ((((leftTailThree.take 5).drop 1).drop 1).drop 1).take 1

private def rightRow :=
  ((((rightTailThree.take 5).drop 1).drop 1).drop 1).take 1

theorem certificateBlockThreeRowThree : leftRow = rightRow := by decide
