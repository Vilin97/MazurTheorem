...
private def leftRow :=
  (((((leftTailThree.take 5).drop 1).drop 1).drop 1).drop 1)

private def rightRow :=
  (((((rightTailThree.take 5).drop 1).drop 1).drop 1).drop 1)

theorem certificateBlockThreeRowFour : leftRow = rightRow := by decide
