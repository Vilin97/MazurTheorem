...
/-!
# Kernel audit for Tau Ceti challenge contracts
...
/-- Import the contract library and run a core action in its environment. -/
def withContractEnvironment {α} (action : CoreM α) : IO α := do
  initSearchPath (← findSysroot)
  unsafe Lean.withImportModules
    #[{ module := `MazurTauCetiChallenge }]
    {}
    (trustLevel := 1024)
    fun environment =>
...
def main (arguments : List String) : IO UInt32 := do
  ...
  for argument in arguments do
    ...
    | .error message =>
        ...
        return 1
  let failures ← withContractEnvironment (audit targets)
