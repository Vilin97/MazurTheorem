...
Snapshot: 2026-08-02, route revision `formal-immersion-at-five-v1`, based on
the owner checkpoint represented locally by `d94496b`. The ledger still has
48 nodes / 1,000 points: one completed 50-point baseline and 47 unfinished
nodes. No weight, completion flag, public node ID, or Challenge was removed.
...
| `codex/owner-o49-tower` | Checked level-seven correspondence and explicit Vélu work | Retain as useful library work, but pivot the endpoint to the direct rational cyclic-subgroup map into the already classified two-cusp `X₀(49)`. Do not require Vélu additivity or a nonbacktracking tower. |
...
1. `MT-TC-A1-ORDER-SUPPORT` (15) → `MT-TC-A2-PRODUCT-FORMULA` (15) →
...
1. `MT-PIN-MIGRATION` (20): clean exact-pin builds, lifecycle and provenance.
