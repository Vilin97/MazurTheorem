...
`CurveDivisorDescent.localLineBundleAddSourceCoordinateIso` expose the named transports from the
proof-defined local bundles to their coordinate presentations, and
`CurveDivisorDescent.localLineBundleAddIso_restrictViaPullback_naturalityViaPairwiseIntersection_global`
proves that the proof-defined global-divisor local addition maps commute with the proof-defined
pairwise-intersection transitions for the two factors and their sum. Assembly into a descent-data
morphism and `DescendedTensorAdditive` remain open. A3 stays `research_open` at weight 18 with
zero credit. Source accounting is 1,046 integrated Lean modules / 1,585,155 lines.
...
eleven.  A new generator-localization theorem now transports that identity
through the actual `Ideal.Fiber.localizedMap`, and the named order-35
consumer has zero remainder.  The special-fibre cusp local-ring generator
and coefficient-unit calculations and modular-chart identification remain
...
Exact source accounting is 1,031 Lean modules / 1,578,321 Lean lines.
...
current exact source baseline is 1,031 Lean modules / 1,578,321 Lean lines.
...
The anti-diagonal norm conic now has a global integral parametrization.
Projection from its rational point produces canonical coprime integers `p,q`;
the noncuspidal cusp equations exclude the tangent and force `p != 0`. The
three cleared conic identities eliminate the square coordinate and place
...
boundary of every global `G_m` point (`ab2f729`). Subcanonical descent now
proves the reverse kernel statement: that boundary is trivial exactly for an
actual `n`th power. Consequently the quotient boundary is injective and gives
...
unconditionally. The cuspidal nonsingular-locus equivalence is now checked by
the finite classification range below. Genuine Néron identity-component
identification and component-cardinality geometry remain open; no node credit
is claimed. The focused Lean targets build with
...
admissible variable change transports the nonsingular locus even for a
singular cubic; finite short normalization identifies the actual additive
groups over `F₅` and `F₁₁`. Lane-specific completion, rational, and
formal-immersion consumers avoid importing the order-35 cone into the
prime-five path. The remaining local premise is exactly the marked assertion
...
The cyclic-quotient range constructs the abstract quotient of the rational
point group by the supplied cyclic subgroup. Its projection is surjective
with exact kernel, multiplication by the level descends to a canonical map
back to the ambient group, both composites are level multiplication, and the
kernel of the descended map is the image of the full level-torsion kernel.
The construction is natural under ambient point-group isomorphisms, and an
actual admissible Weierstrass variable change is the compiled
`RationalDatum` consumer. Independent review accepted both orientations and
the variable-change inverse. This is not a construction of the elliptic
quotient scheme or its dual isogeny; those remain open, and no node credit is
claimed (`48a3207`, `f1ad4db`).
...
The next affine-cusp layer no longer accepts an arbitrary linear functional.
A chosen localized parameter which generates the cusp maximal ideal and
survives modulo its square proves exact cotangent dimension one. A concrete
first-order identity for the pulled-back quotient parameter, with a unit
coefficient of q and a remainder in the square, constructs a genuinely
detected cotangent vector and proves formal immersion of the actual
...
The local reduction range proves directly that multiplicative reduction
forces valuation of `j` strictly above one; integral `j` therefore excludes
that branch. Mathlib's exhaustive reduction-type trichotomy and the checked
tame-additive component/residue/formal-kernel obstruction then prove good
reduction from a genuine supplied Néron filtration (`9077ee1`). Independently,
the exhaustive 121-model `F₁₁` bound now has a separated point-order API:
every point has order at most eighteen, every exact order at least nineteen is
excluded, and exact order 35 is consumed by the existing integral
good-reduction specialization (`67d2ab2`). The completed local equation is now
consumed directly: Mathlib's selected integral model over the five-adic
valuation ring is proved elliptic, `adicRed` preserves the marked exact order,
...
Ceti abelian variety and a multiplicative comparison of rational points into
the narrow `WeierstrassGroupSchemeInterface`; exact torsion is a compiled
downstream consumer producing the split finite-flat `Γ₀(N)` datum, with a
checked theorem recovering the coordinate cyclic subgroup.

Checkpoints `69ce0b1` and `a711633` prove the two reverse `H¹` inclusions. A
trivializing middle-group zero-cochain has compatible quotient images that
glue to a global boundary section. Conversely, a quotient-valued trivializing
gauge lifts after pulling back the actual fppf quotient cover; gauging the
middle cocycle then gives a strict kernel-valued cocycle by certified kernel
uniqueness. The concrete six-term constructor consumes both theorems and now
accepts no exactness hypotheses.

Checkpoints `09a9c00` and `aef49ec` construct the homogeneous Weierstrass
cubic as the reduced closed subscheme of `P²`, prove its defining polynomial
is homogeneous of degree three, identify the closed-immersion range, and prove
the cubic is reduced and proper over the base field. Tau Ceti's abelian-variety
constructor now consumes this exact scheme once its group object and geometric
integrality are supplied; the existing split finite-flat `Γ₀(N)` datum is a
compiled downstream consumer after the projective coordinate-point comparison.

Checkpoint `b3cfcaf` transports the canonical trivial descent datum to the
...
`Γ₀(N)` data once this explicit map is proved multiplicative.
...
morphism. Reviewed checkpoint `ed73a67` proves that unit-equivalent
normalizations have the same ambient closed point, that constants and the
three coordinate ratios determine every chart ring map, and hence that the
...
reconstructed normalized principal cocycle with the directly transported datum.
The actual reconstructed principal Picard datum therefore has the global trivial
line bundle as a checked effectivity witness. Arbitrary-divisor coherence and
...
wrapper has checked finite-flat compatibility consumers. Ambient
...
1. Completed `MT-TC-A1-ORDER-SUPPORT` (15) and
   ...
   of an integral curve intersect. Pinned Tau Ceti valuation rings now prove
...
6. In parallel, narrow `MT-EC-ISOGENY-WEIL` (25) to rational cyclic
   ...
   kernels actually consumed by `X₀(N)` and reduction. The abstract point
   quotient now has the exact supplied kernel; multiplication by the level
   descends, both quotient/dual composites are level multiplication, and the
   dual kernel and admissible-variable-change naturality compile with a
   `RationalDatum` consumer. Construct the represented elliptic quotient and
   its base-change law next. A general Weil-pairing theory is not an
   acceptance target without a named consumer.
...
4. Completed `MT-FFGS-BASIC` (20): the exact supplied fppf quotient,
   ...
   a supplied unrelated group law. Coefficient natural transformations now
   descend to global homomorphisms with identity/composition laws, actual
   finite-flat group-scheme morphisms consume them, and certified kernels are
   ...
   compiles and is consumed by the constructor. On the other `H¹` term, the
...
1. `MT-X0-MODULI` (30): the raw split rational cyclic subgroup/datum, transport by
   ...
   order-five and order-seven carriers reconstruct `C` also compile. The
...
2. `MT-X0-INTEGRAL` (30): compactification and cusp neighbourhoods only in
   ...
   the base field; the image residue field is now also derived structurally
   when the quotient lies over the base. A nonzero simultaneous
   Hecke-eigen-expansion feeds formal immersion all the way to equality of
   arbitrary Noetherian local sections with equal quotient image. On an
   explicit affine cusp chart, a principal maximal ideal generated by a
...
4. `MT-X0-JACOBIAN` (20) → `MT-X0-HECKE` (30): cusp-normalized Abel--Jacobi,
   ...
   all the way to equality of canonical local-spectrum maps. The abstract
   Hecke argument now proves that a nonzero simultaneous eigen-expansion has
   nonzero first coefficient and is consumed by the real formal-immersion
   predicate. This is partial infrastructure only:
...
- `MT-O35-EXCLUDE` (14) constructs the explicit optimal elliptic quotient
  ...
  cusp section, exact first-order quotient identity, and its genuine
  localized-map transport now compile. The characteristic-eleven local-ring
  generator and coefficient-unit calculations, represented modular quotient/
  classifying map, and Atkin--Lehner bad-fibre specialization remain
  open. No order-35
  node credit is claimed
...
- `MT-O49-TOWER` (10) now has a checked endpoint from the variable-change
  quotient of the subgroup generated by a point of exact order `49` to the
  checked two-cusp model. Construct the actual coarse classifying map, prove
  its image is noncuspidal, and identify the target with `X₀(49)`; no Vélu
  additivity or nonbacktracking tower is needed.
