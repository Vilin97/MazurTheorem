...
- `MT-O35-EXCLUDE` (14) constructs the explicit optimal elliptic quotient
  ...
  trivial torsion on its candidate target now compile. This is not rank zero:
  additivity or a bundled isogeny, the dual/composition, and both
  three-isogeny Selmer images remain. The local normalization/enumeration,
  good-reduction join,
  and an abstract tame-additive filtration consumer also compile. The actual
  modular quotient map, rank-zero proof, formal immersion, and geometric
  bad-fibre specialization remain open. Once good reduction at eleven is
  supplied, the joined theorem
