...
| Represented `X₀(N)` vertical slice | `MT-EC-ISOGENY-WEIL`, `MT-X0-MODULI`, `MT-X0-INTEGRAL`, `MT-X0-JACOBIAN` | `WP-MT-EC-ISOGENY-WEIL-EXACT-TORSION-SUBGROUP` | Construct the finite-flat cyclic subgroup generated by an exact-torsion point and refactor `MazurTorsion.XZeroFortyNine.rationalDatumOfSplitFiniteFlatSourceOfOrderFortyNineTorsion` to consume it without an arbitrary `M : WeierstrassGroupSchemeInterface E` input. | Represented `X₀(N)` geometry reaches the integral-model and Jacobian layers without supplied point-equivalence shadows. |
...
The represented-moduli lane now advances through the selected exact-torsion
finite-flat subgroup package. The generic Néron mapping-property and
