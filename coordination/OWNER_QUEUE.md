...
middle-`H¹` exactness for supplied fppf quotient presentations,
...
Weierstrass group-scheme interface,
...
The combined owner history through `4b6a54b` contains two newly reviewed
acceptance boundaries. Checkpoint `8f77d87` proves exactness at quotient-valued
...
Reviewed source checkpoint `4b53d76` proves exactness at middle global `H¹`:
a quotient-valued gauge trivializing the projected cocycle is lifted after
pullback to the genuine fppf project cover, and certified-kernel uniqueness
gives the required kernel-valued cocycle. The concrete low-degree constructor
now supplies this exactness internally as well.

All three checkpoints passed independent mathematical and API review unchanged.
The focused single-threaded builds of `FppfQuotientConnecting` and
`XZeroWeierstrassAbelianVariety` pass; the audited declarations use only
`propext`, `Classical.choice`, and `Quot.sound`; all 11 Challenge contracts are
unchanged; and `python3 scripts/quality.py` passes at 719 integrated Lean
sources / 1,458,666 lines with all 48 node IDs and 1,000 points intact. No
checkpoint claims roadmap-node completion credit.

The remaining modular boundary is to construct the actual projective cubic as
...
independent descent routes were audited for the degree-one terms: the lifted
quotient-gauge construction now proves middle-`H¹` exactness, while the
kernel-`H¹` reverse inclusion remains the only missing exactness input before
the focused finite-flat rank-zero consumer.
...
4. Completed `MT-FFGS-BASIC` (20): the exact supplied fppf quotient,
   ...
   second refinement pulls back a quotient-valued trivializing gauge to the
   project cover; the uniquely kernel-valued gauge transform proves exactness
   at middle `H¹`, and the constructor consumes that proof too. Next finish
   `MT-FFGS-CONNECTED-ETALE` (20) by proving reverse exactness at kernel `H¹`
   and filling the six-group/five-map exact-sequence instance. Then finish
...
1. `MT-X0-MODULI` (30): the raw split rational cyclic subgroup/datum, transport by
   ...
   consumer. Next construct that projective cubic abelian variety and point
   comparison for the elliptic curve, then `E/C` and the coarse `X₀(N)`
...
- `codex/owner-ffgs-kummer`: prove reverse exactness at kernel `H¹` for the
  reviewed quotient-derived boundary. Degree-zero and middle-`H¹` exactness
  now compile and are consumed by the concrete low-degree constructor.
