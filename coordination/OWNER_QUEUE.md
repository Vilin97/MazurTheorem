...
- `MT-O35-EXCLUDE` (14) constructs the explicit optimal elliptic quotient
  ...
  squarefree-level formal immersion at auxiliary prime `11`. The local
  normalization/enumeration and its exact-order-35 fibre consumer now compile
  as `OrderThirtyFive.card_reductionAtEleven_le_eighteen` and
  `reductionAtEleven_addOrderOf_ne_thirtyFive`; the quotient, rank-zero proof,
  formal immersion, and bad-fibre specialization remain open. Once good
  reduction at eleven is supplied, the joined theorem
