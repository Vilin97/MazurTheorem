...
* `theorem` (`contract`):
  ...
  classes.
* `definition` (`contract`):
  `MazurTorsion.Kubert.orderTwentyFiveCuspidalSubgroupZModEquiv`
  Identify the subgroup generated by all five based cusp differences with the cyclic
  additive group Z/71Z when adjacent cusp classes are distinct.
* `definition` (`contract`):
