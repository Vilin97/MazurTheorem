...
This canonical target must next be identified with the already checked
blockwise monic-coefficient chart.  The remaining local theorem must prove
that identification (equivalently, prove that this finite étale local
presentation recovers the prescribed monic invariant-ring chart), identify the
graph-product ideal with
