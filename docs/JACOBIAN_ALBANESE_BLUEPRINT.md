...
| Equivariant fpqc refinement and sheet transitions | `EquivariantFpqcRefinement.refinement`, `refinement_isAffine_finiteEtale_fpqc`, `projection_one_comp_cover`, `rawActionHom_comp_cover`, `rawActionHom_mul`, and `refinementAction`; `EquivariantSplitRefinement.splitIso`, `directAction`, `splitAction`, and `splitAction_equivariant`; `SplitFiniteActionSheets.sheetTransition`, `action_sheetPoint`, `sheetTransition_isLocallyConstant`, `sheetTransition_mul`, `fixedSheetPerm`, `constantTransitionSet_isClopen`, `constantTransitionOpen_isStable`, and `sheetMap_eq_on_stableOpen`; `GeometricAssignedAffineChart.componentFpqcBlockRefinement`, `componentFpqcBlockSplitIso`, `componentFpqcBlockSplitAction`, `componentFpqcBlockSheetTransition_mul`, and `componentFpqcBlockFixedSheetPerm`; `PointedIncidenceDescent.orderedSupportGeometricAssigned_blockRefinement_isAffine_finiteEtale_fpqc`, `orderedSupportGeometricAssigned_blockRefinementSplitIso`, and `orderedSupportGeometricAssigned_blockRefinementSplitAction` | checked: the finite relative product of every translate of an affine finite étale fpqc cover is again affine finite étale fpqc; its identity-indexed projection refines the original cover, and right translation of the product indices defines a genuine action covering the target action. Pulling an equivariant finite source and its split presentation to the common refinement induces an action on the direct pullback; transport through the slice isomorphism gives a genuine action on the split finite source whose projection is equivariant. Every group element and refinement point now has a uniquely extracted destination sheet; these labels are locally constant and satisfy the action cocycle over translated base points. At a fixed refinement point they are genuine sheet permutations, and simultaneous agreement with those permutations defines a stable clopen neighborhood. On every sheet of that neighborhood the pointwise label is upgraded to an exact scheme-morphism equation: restricted base action followed by the selected target-sheet inclusion. Constructing a suitable fixed lift and comparing its resulting permutation representation with the prescribed blockwise model remain |
...
point; it satisfies the translated-base cocycle law, and at any fixed
refinement point it gives a genuine permutation of the finite sheet set.
These transition functions are now proved locally constant; simultaneous
agreement with their values at a fixed point cuts out a clopen neighborhood
which is itself stable under the group action.  On each source sheet over
this neighborhood, factorization through the constant target sheet and the
structural projection prove an exact equality of scheme morphisms with the
restricted base action.  The next step is to produce a suitable fixed lift
in the refinement and compare its finite permutation representation with
the prescribed blockwise model.  The quotient of
