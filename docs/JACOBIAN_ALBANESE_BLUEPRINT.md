...
ideal-identification and invariant-descent problem: construct the morphism
from this support coproduct into the curve-level ordered ambient, identify
each pulled-back coordinate graph with its owner occurrence graph (and rule
out off-owner equality loci on the chosen neighborhood), then use the
intrinsic product equality above.  Afterward compare the selected
...
The precise remaining step is therefore narrower still: construct the map
from this coproduct to the curve-level ordered ambient and prove that every
pulled-back coordinate graph is the corresponding owner graph on a
neighborhood excluding all off-owner equality loci.  The restricted
