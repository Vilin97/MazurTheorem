...
| Support-coproduct monic root algebra | `SectionProductRootAlgebra.piIdealQuotientAlgEquiv`, `sectionPolynomial`, `graphProductQuotientEquivRoot`, and `graphProductQuotient_finrank`; `AffineLineSectionProduct.polynomialCoordinateAlgEquiv`, `map_sectionGraphProductIdeal`, and `sectionGraphProductQuotientEquivRoot`; `GeometricAssignedAffineSupportCoproduct.supportPieceGraphProductQuotientEquivRoot`, `supportCoproductAffineSchemeIso`, `supportCoproductToProductSectionsAlgEquiv`, `supportCoproductGraphProductIdeal`, `supportCoproductGraphProductQuotientEquivProduct`, and `supportCoproductGraphProductQuotient_finrank`; `PointedIncidenceDescent.orderedSupportGeometricAssigned_supportCoproductGraphProduct_finiteFlat_rank` | checked: polynomial coordinates send the kernel of a section with value `x_i` to `(X - x_i)`, so the complete product quotient is the monic root algebra for `∏_i (X - x_i)`. Transport through the standard relative-affine-line isomorphism and then through the isolated étale support coordinate identifies each geometric support-piece quotient with this algebra. The affine finite coproduct is canonically the spectrum of the product of the summand section rings; the induced algebra equivalence is proved componentwise to be the actual restriction map. Pulling back the product of the summand ideals therefore defines an ideal sheaf on the actual support coproduct, and quotienting commutes with both the finite ring product and `ULift` reindexing. The resulting coproduct quotient is finite flat of rank `d`; repeated sections contribute repeated polynomial factors and never lose nilpotent multiplicity. |
...
as the ordered-incidence consumer.

The remaining local comparison is therefore only an ideal-identification
and invariant-descent problem: prove that pulling the curve-level ordered
incidence ideal through the simultaneous affine chart gives exactly
`supportCoproductGraphProductIdeal`, then compare its selected block-invariant
quotient with the block-monic coefficient chart.  Once that comparison is
cartesian, the existing pointwise fpqc interfaces descend the isomorphism,
flatness, rank, and Cartier equation to the universal effective divisor.
...
The precise remaining step is narrower: identify the quotient of the actual
support coproduct by the restricted curve-incidence ideal with this checked
product algebra, then descend that identification through the selected-sheet
invariant quotient.
