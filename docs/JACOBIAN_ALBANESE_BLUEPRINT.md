...
quotients.  The quotient of this selected-sheet open by
`∏_j S_{n_j}` must then be identified with the already checked blockwise
monic-coefficient chart.  The remaining local theorem must prove that
quotient comparison (equivalently, prove that this finite étale local
presentation recovers the prescribed invariant-ring chart), identify the
