...
| Two-layer finite-support correction | `FiniteSupportIndex.topologicalSupportFinset`, `topologicalCoordinateSupportIndex_eq_iff`, `sum_topologicalSupportMultiplicity`, `supportFinset`, `coordinateSupportIndex_eq_iff`, `residuePointToPower_comp_distinct_comp_repeat`, `FiniteSupportEtaleCoordinates.geometricResiduePoint_reconstruction`, `distinctSupportAssignedProductToCoproductPower`, `distinctCharts`, `distinctNeighborhoods`, `PointedIncidenceDescent.orderedSupportDistinctPoint`, and `sum_orderedSupportMultiplicity` | checked: the topological finite image indexes one local neighborhood per underlying curve point, with a surjective occurrence assignment and multiplicities summing to the full degree; separately, the finite image of actual coordinate morphisms from the residue-field spectrum records residue-field embeddings, and geometric representative selection followed by repetition recovers the exact original residue-field point; the assigned-product map retains the original degree while inserting every occurrence through its topological neighborhood member |
...
image chooses a chart and finite étale neighborhood once per underlying curve
point.  Its occurrence assignment is surjective, its equality criterion is
equality of underlying points, and its fiber cardinalities sum to `d`.  The
geometric finite image instead enumerates the actual coordinate morphisms
from the common residue-field spectrum.  Selecting one representative of
each geometric morphism and repeating it by the geometric occurrence map is
proved to recover the original residue-field point exactly.  Thus neither
artificial neighborhood copies nor residue-field correlations are discarded.
`FiniteSupportEtaleCoordinates.distinctCharts` and `distinctNeighborhoods`
use the topological layer, while `geometricResiduePoint_reconstruction`
records the exact layer.  The assigned-product construction keeps the family
cardinality independent of degree and inserts each of the `d` occurrences
through its topological member.  The remaining geometric work must migrate
the common-base/fpqc splitting and exact support-lift construction to that
assigned product; only then may the resulting sheet multiplicities feed the
monic-root algebra.
...
remains in this local layer is the geometric comparison showing that the
curve-level ordered incidence family, after restriction to the chosen finite
étale split chart, is this explicit product algebra; its fpqc descent data
must then be matched with the global pointed comparison.
