...
étale occurrence-coordinate morphism, identify the restricted curve-level
graph ideal with `SplitComponentGraphIdeal.orderedGraphIdeal`, and descend
the resulting isomorphism to the actual curve quotient.  The algebra after
that transport is no longer missing: `SplitComponentGraphBaseChange` proves
the required equality after an arbitrary coefficient change, while
