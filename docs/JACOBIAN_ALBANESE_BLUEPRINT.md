...
| Affine occurrence-wise block chart | `AssignedProductStabilizer.action`, `assignedMap_equivariant`, `productToAssignedProduct_comp_assignedMap`, `RelativePowerBaseIso.powerIso`, `powerPoint`, `assignmentRepeatHom_powerPoint`, `GeometricAssignedAffineChart.affineComponentFamily`, `commonAffineComponent`, `componentToBasePower`, `exactCommonAffineBasePoint`, `componentToCoordinatePower`, `componentToCurvePower`, `componentToCurvePower_commonAffineComponentPoint`, `componentToBasePower_rankOpen_isStable`, `exists_componentToBasePower_affineOpen_fpqc_splitCover`, `componentFpqcBlockRefinement`, `componentFpqcBlockRefinement_isAffine_finiteEtale_fpqc`, `componentFpqcBlockRefinement_projection_one_comp_cover`, `componentFpqcBlockRefinement_actionMap_comp_cover`, `componentQuotientToBaseQuotient`, `componentQuotientToBaseQuotient_comp_quotientToCoordinateQuotient`, and their `PointedIncidenceDescent` consumers | checked: the affine coordinate base and the curve chart are kept distinct; each chosen finite-étale component is restricted over an affine base open through its support point, and one independent copy of that actual curve component is used for every ordered occurrence; the resulting degree-`d` product is affine and finite étale over its affine occurrence-base product, and its equivariant map to ordered affine-root space factors through that base; postcomposition along the ground-field base isomorphism canonically preserves finite powers and commutes with the occurrence assignment; the finite-product local-ring lifting theorem therefore constructs one correlated point in the product of distinct selected components mapping to the exact geometrically deduplicated curve tuple, and repeating it maps to the exact original ordered divisor point rather than merely agreeing coordinatewise; its image in the occurrence base retains that correlation and is block-fixed; fiber rank is invariant under the block action, and the exact point has a block-stable affine neighborhood inside its constant-rank locus whose source preimage is block-stable and becomes a finite set of the exact fiber rank after an affine fpqc cover; the common product of all block translates remains affine finite étale fpqc, refines the original cover, and has block-translation maps covering the base action; the component-to-base map descends between finite block quotients, and the quotient coordinate map factors through this descended map |
| Dimension-preserving affine root graphs | `AssignedProductStabilizer.residuePoint_productToAssignedProduct_projection_eq`, `residuePoint_map_toProductToAssignedProduct_projection_eq_to_target`, `FiniteSupportIndex.residue_coordinates_ne_of_supportIndex_ne`, `ResidueEqualizerOpen.complement`, `mem_complement_of_residue_ne`, and `residue_ne_at_image_of_comp_ne`; `GeometricAssignedAffineChart.residuePoint_map_toCommonAffineComponent_projection_eq_to_support`; `GeometricAssignedAffineRootCoordinates.tupleSheetSection_comp_structure`, `tupleSheetToComponentPreimage_eq_direct`, `exists_tupleSheetToComponentPreimage_over_exact`, `tupleSheetToOccurrenceComponent_apply_of_exact`, `tupleSheetToOccurrenceComponent_comp_coordinate`, `occurrenceCoordinate_comp_coordinateLine`, `occurrenceAmbientIsoSupportAmbient`, `exists_occurrenceOpen_graphIdeal`, and `exists_supportOpen_graphIdeal`; `GeometricAssignedSimultaneousGraphNeighborhood.supportIntersectionOpen`, `simultaneousBaseOpen`, `residue_graphToSupportAmbientAtSupport_eq_of_exact`, `mem_simultaneousBaseOpen_of_exact`, `graphToSupportIntersection`, `supportIntersectionBaseChange`, `supportIntersectionGraph_isPullback`, and `supportIntersection_equalCoordinateIdeal`; `GeometricAssignedAffineSimultaneousNeighborhood.SimultaneousAffineGraphNeighborhood`, `nonempty_simultaneousAffineGraphNeighborhood_of_exact`; `EqualCoordinateClosedImmersion.equalCoordinate_relativeSection_square`, `relativeSection_ker_comap_ambientToRelativeTarget`; `EtaleQuotientProduct.extendedQuotientMap_bijective_mul_of_inf`; `GeometricAssignedAffineSupportCoproduct.supportCoproduct`, `supportCoproduct_isAffine`, `supportPiece_equalCoordinateIdeal`, `supportPieceGraphProductIdeal_top_eq_map`, `supportPieceToSupportAffineLine_etale`, `graphToSupportPiece_comp_supportAffineLine`, `supportAffineLineGraph_ker_comap_supportPiece`, `supportPieceGraphProductIdeal_eq_coordinateSection_comap`, `supportPieceGraphProductIdeal_top_eq_map_coordinateSections`, `supportPiece_extendedQuotientMap_bijective_mul_of_inf`, `supportCoproductToAffineComponentCoproduct`, `supportCoproductToCoordinateOrderedAmbient`, `graphToCoordinateGraph`, `crossSupportDistinctBaseOpen`, `supportPieceOwnerOnlyOpen`, `CrossSupportAffineGraphRefinement`, `nonempty_crossSupportAffineGraphRefinement`, and their `PointedIncidenceDescent` consumers | checked: on the common block-refinement cover, every sheet of the split occurrence-component product is a section over the refinement base and projects to a genuine selected affine curve component at each occurrence. The direct and iterated split presentations agree, so a sheet chosen over the exact correlated divisor point really maps to the exact repeated component point. Its affine-line coordinate is exactly the corresponding coordinate of the dimension-`d` occurrence base, so repeated central support points remain independently deformable rather than being specialized to one value. Each independently varying graph has an isolating open on which the equal-coordinate ideal is exactly the graph ideal; because all occurrence coordinates have the same structural composite to the coordinate ground scheme, these opens transport to the single affine curve-family ambient indexed by their common support owner while preserving both the pullback square and the exact ideal equality. A union contains the graphs but does not preserve every ideal equality. The simultaneous construction therefore intersects the isolating opens and restricts the refinement base to the locus where every selected graph lands in that intersection; after base change, every occurrence graph is a genuine section of its support neighbourhood. Residue-field correlation of the exact repeated-product point proves that all same-support graph morphisms agree on the residue-field point, including its tensor-product prime rather than only its two visible projections. Hence the exact tuple-sheet lift belongs to every isolating open and to the simultaneous base open. Pulling a selected graph along the support intersection preserves its cartesian square, so the equal-coordinate ideal on that intersection is still exactly the restricted graph ideal. Affine-basis shrinking then supplies one affine support open per geometric support member and a final affine base open through the exact lift on which every occurrence graph lands in its assigned support piece. Pulling those pieces to the final base and taking their finite coproduct gives one affine common curve-family chart; every occurrence graph is a section into precisely its owner summand, the comparison to the original selected-component coproduct preserves that owner, and each equal-coordinate ideal remains the exact pulled-back graph ideal on the affine support piece. The coproduct is paired with the chosen tuple-sheet map into the coordinate-base ordered ambient; its first projection is the varying curve point and its second projection is the full ordered curve tuple. Every owner occurrence graph equalizes the first projection with its matching coordinate and therefore factors canonically through the genuine curve-level coordinate graph. Distinct geometric-support indices now give provably unequal residue-field coordinate maps at the exact lift. For a separated curve, the equalizer images are closed; their complements can therefore be intersected on the base and on each support piece. A second affine-basis shrinking produces `CrossSupportAffineGraphRefinement`, whose base excludes all cross-support coordinate collisions and whose support member `a` excludes equality with every coordinate not owned by `a`, while every owner graph and the exact lift survive. This is scheme-theoretic separation, so it remains valid even when two distinct geometric coordinates have the same underlying topological curve point. The support-piece map to the common affine line is proved étale by exhibiting it as two successive base changes of the selected curve coordinate, with the iterated pullback flattened by the canonical pullback-pasting isomorphism. Composing an occurrence graph with this map is exactly its independent affine-line section. A reusable cartesian relative-section theorem then identifies every geometric graph ideal with the pullback of that section ideal; finite products and affine global sections preserve the identification, including every repeated occurrence factor. Formal étaleness supplies the collision step: if extension induces an isomorphism after quotienting by the intersection of two root-section ideals, then it induces one after quotienting by their product. The proof lifts the inverse uniquely across the square-zero thickening from product to intersection, so coincident roots retain their nilpotent multiplicity rather than being reduced |
| Support-coproduct monic root algebra | `SectionProductRootAlgebra.pi_fintype_prod_single`, `piIdealQuotientAlgEquiv`, `sectionPolynomial`, `graphProductQuotientEquivRoot`, and `graphProductQuotient_finrank`; `AffineLineSectionProduct.polynomialCoordinateAlgEquiv`, `map_sectionGraphProductIdeal`, and `sectionGraphProductQuotientEquivRoot`; `GeometricAssignedAffineSupportCoproduct.supportPieceGraphProductQuotientEquivRoot`, `supportCoproductAffineSchemeIso`, `supportCoproductToProductSectionsAlgEquiv`, `graphToSupportCoproduct_ringHom_ker_eq_comap_pi`, `supportCoproductOccurrenceGraphProductIdeal_eq_assembled`, `supportCoproductGraphProductQuotientEquivProduct`, and `supportCoproductGraphProductQuotient_finrank`; `PointedIncidenceDescent.orderedSupportGeometricAssigned_supportCoproductGraphProductIdeal_eq_kernels` and `orderedSupportGeometricAssigned_supportCoproductGraphProduct_finiteFlat_rank` | checked: polynomial coordinates send the kernel of a section with value `x_i` to `(X - x_i)`, so the complete product quotient is the monic root algebra for `∏_i (X - x_i)`. Transport through the standard relative-affine-line isomorphism and then through the isolated étale support coordinate identifies each geometric support-piece quotient with this algebra. The affine finite coproduct is canonically the spectrum of the product of the summand section rings; the induced algebra equivalence is proved componentwise to be the actual restriction map. Under those product coordinates, the kernel of one occurrence graph is its owner-piece kernel in the owner component and the unit ideal in every other component. Multiplying over all support owners and all their occurrences proves that the assembled ideal sheaf is intrinsically the scheme-theoretic product of the actual occurrence-graph kernels on the coproduct, rather than merely a transported product-ring definition. Quotienting commutes with both the finite ring product and `ULift` reindexing. The resulting coproduct quotient is finite flat of rank `d`; repeated sections contribute repeated polynomial factors and never lose nilpotent multiplicity. |
...
| Constant-rank neighborhoods | `FiniteFlatConstantRankNeighborhood.rankOpen`, `morphismRestrict_finrank`, `EquivariantFiniteEtalePointSplitChart.finrank_action`, and `rankOpen_isStable` | checked: the rank fiber through a chosen point is clopen, contains that point, and restriction of a finite flat finitely presented morphism to it has constant rank; for an equivariant finite étale map, the action preserves fiber rank and hence the whole rank locus |
| Finite étale split chart | `EtaleSplitChart.splitProjection`, `EtaleSplitChart.splitProjection_finrank`, `EtaleSplitChart.exists_fpqc_splitCover`, `AffineFiniteEtaleSplitChart.exists_fpqc_splitCover`, `FiniteEtalePointSplitChart.exists_affineOpen_fpqc_splitCover`, `EquivariantFiniteEtalePointSplitChart.exists_stableAffineOpen_fpqc_splitCover`, `FiniteEtaleFamilySplitChart.exists_common_splitCover`, `AffineFiniteEtaleFamilySplitChart.exists_fpqc_common_splitCover`, and `FiniteEtaleFamilyPointSplitChart.exists_affineOpen_fpqc_common_splitCover` | checked: after a finite faithfully flat étale extension, a constant-rank finite étale algebra becomes a finite product of the base; a finite family has, around every common base point, one affine neighborhood in the intersection of all its clopen rank loci and one finite étale fpqc cover splitting every restricted member; at a fixed point of an equivariant map, the affine neighborhood can be chosen stable with stable source preimage; the scheme isomorphisms commute with the projection to the fpqc base, and on the pointwise nonempty charts the number of split sheets is proved equal to the original fiber rank |
| Equivariant fpqc refinement and sheet transitions | `AffineClopen.isAffineOpen_of_isClopen` and `isClosedImmersion_ι_of_isClopen`; `FiniteGroupQuotient.localQuotientπ_surjective`; `EquivariantFpqcRefinement.refinement`, `refinement_isAffine_finiteEtale_fpqc`, `projection_one_comp_cover`, `rawActionHom_comp_cover`, `rawActionHom_mul`, and `refinementAction`; `EquivariantSplitRefinement.splitIso`, `directAction`, `directSplitIso`, `exists_splitSheet_over_point`, `splitAction`, `splitAction_equivariant`, `splitAction_toOriginal_equivariant`, and the finite étale direct-base-map instances; `SplitFiniteActionSheets.sheetTransition`, `sheetTransitionPerm`, `action_sheetPoint`, `sheetTransition_isLocallyConstant`, `sheetTransition_mul`, `orbitTransitionOpen_isStable`, `sheetMap_eq_on_open`, `sheetMap_eq_on_constantTransitionOpen`, `orbit_sheet_maps_fixed_point`, `selectedOrbitSheetOpen`, `selectedOrbitSheetOpen_isStable`, `selectedOrbitSheetOpen_isAffine`, `selectedOrbitSheetAction`, `selectedOrbitSheetQuotient`, `selectedOrbitSheetToOriginal`, `selectedOrbitTargetOpen`, `selectedOrbitTargetOpen_isStable`, `selectedOrbitSheetToTargetOpen_isFinite_etale`, `selectedOrbitSheetToTargetOpen_surjective`, `selectedOrbitTargetQuotient`, `selectedOrbitSheetTargetDescendedMap`, `selectedOrbitTargetLocalQuotient`, `selectedOrbitSheetQuotientπ_comp_targetLocalDescendedMap`, and `selectedOrbitSheetTargetLocalDescendedMap_surjective`; `GeometricAssignedAffineChart.componentFpqcBlockRefinement`, `exists_componentFpqcBlockRefinementPoint_over_exact`, `componentFpqcBlockRefinement_orbit_maps_to_exact`, `componentFpqcBlockSplitIso`, `exists_componentFpqcBlockSheet_over_exact`, `componentFpqcBlockSplitAction`, `componentFpqcBlockOrbitTransitionOpen_isStable`, `componentFpqcBlockSelectedOrbitSheetOpen_isStable`, `componentFpqcBlockSelectedOrbitTargetOpen_isStable`, `componentFpqcBlockSelectedOrbitSheetToTarget_properties`, `commonAffineComponentPointInPreimage_mem_selectedOrbitTargetOpen`, and `componentFpqcBlockSelectedOrbitTargetLocalDescendedMap_surjective`; `PointedIncidenceDescent.orderedSupportGeometricAssigned_blockRefinement_isAffine_finiteEtale_fpqc`, `orderedSupportGeometricAssigned_blockRefinementSplitIso`, and `orderedSupportGeometricAssigned_blockRefinementSplitAction` | checked: the finite relative product of every translate of an affine finite étale fpqc cover is again affine finite étale fpqc; its identity-indexed projection refines the original cover, and right translation of the product indices defines a genuine action covering the target action. Pulling an equivariant finite source and its split presentation to the common refinement induces an action on the direct pullback; transport through the slice isomorphism gives a genuine action on the split finite source whose projection and map back to the original source are equivariant. Every group element and refinement point has a uniquely extracted destination sheet; these labels are locally constant and satisfy the action cocycle over translated base points. A fixed refinement lift is neither assumed nor generally available. Instead, any lift of the exact fixed base point has a finite orbit above that same point; the union of the transition-signature loci along this orbit is stable and clopen, and each signature piece carries exact sheetwise action equations. Pullback-carrier surjectivity selects an actual sheet over the chosen lift mapping to the exact selected-component point, and equivariance proves that every translated sheet maps to that same fixed source point. The corresponding orbit of source sheets over those signature pieces is itself a stable clopen affine open. Its image in the original component chart is again stable and clopen, contains the exact selected-component point, and is affine when the component preimage is affine. The selected-sheet map onto this image is surjective finite étale and equivariant, so both restricted actions have finite-group quotients and the map descends between those local quotients. The target quotient is also constructed in its canonical affine form `Spec Γ(U)ᴳ`; the selected-sheet quotient maps surjectively to it and satisfies the defining quotient square. Comparing this restricted component target with the pullback of the global blockwise monic coordinate chart remains |
...
| Geometric block-monic coordinate quotient | `RelativeAffineLinePower.linePowerSchemeIsoPolynomial`, `AssignedTupleMonicCoordinates.monicCoefficientInvariantSchemeIso`, `GeometricBlockMonicChart.coordinateQuotientIsoMonicCoefficientSpace`, `quotientπ_comp_baseQuotientToMonicCoefficientSpace`, `componentQuotientπ_comp_componentQuotientToMonicCoefficientSpace`, `FiniteGroupQuotient.existsUnique_localQuotientπ_lift`, and `GeometricLocalBlockMonicChart.selectedSheetQuotientπ_comp_monic` | checked: the finite relative power of the affine line is the multivariable polynomial spectrum with exact coordinate projections and permutation equivariance; for an arbitrary support-assignment tuple its stabilizer is conjugated to the product of the symmetric groups of the assignment fibers, so its invariant ring is freely generated by the blockwise monic coefficients; categorical quotient uniqueness identifies Mazur's geometric ordered-coordinate quotient with this explicit coefficient space, and the actual affine-base and selected-component quotient maps satisfy the corresponding Vieta squares; invariant maps from stable affine opens factor uniquely through their canonical local invariant-ring quotients, so the selected target neighbourhood and its orbit-sheet quotient now map to the same monic coefficient space with exact defining squares |
...
| Split-component graph-product quotient and base change | `SplitComponentGraphIdeal.orbitAdaptedPositionEquivRootIndex`, `rootGraphEvaluation_ker`, `orderedGraphIdeal_eq_orderedRootLocusIdeal`, `orderedGraphIdeal_eq_ker`, `SplitComponentGraphQuotient.orderedGraphQuotientEquivOrderedRoot`, `orderedRootLocusIsoGraphLocus_hom_comp_ι`, `SplitComponentInvariantRootComparison.graphLocusIsoOrderedRootLocus_hom_comp_ι`, `SplitComponentGraphBaseChange.orderedAmbientBaseChangeHom_map_orderedGraphIdeal`, `baseChangedGraphQuotientEquivOrderedRoot`, and `baseChangedGraphQuotient_finrank` | checked: an actual ordered tuple is reindexed by its sheet fibers using the permutation witnessing its relation to the component's arbitrary orbit representative; one root graph is exactly “select the owner sheet, then evaluate at the root,” with kernel the linear factor on that sheet and the unit ideal elsewhere; the product of all graph ideals is the exact kernel of simultaneous evaluation into the sheetwise ordered-root algebras, so repeated coordinates retain their scheme-theoretic factor multiplicity; the first isomorphism theorem identifies the quotient by that actual graph-product ideal with the ordered split-root algebra, and the induced scheme isomorphism commutes with both closed immersions into the common sheetwise affine-line ambient space. After an arbitrary coefficient-ring map, the entire graph product maps exactly to the transported graph product, whose quotient is the product of the transported monic root algebras and is finite free of total rank `d` |
| Geometric assigned root specialization | `GeometricAssignedRootCoordinates.positionRootOwner_productPoint`, `rootSheetToChart_comp_schemeMap`, `rootSheet_exists_graphCoproduct`, `baseCoordinate_comp_coordinateLine`, `evaluatedRootGraphEvaluation_ker`, `evaluatedRootGraphEvaluation_orderedAmbientCoordinateHom`, `orderedAmbientCoordinateHom_map_orderedGraphIdeal`, `GeometricAssignedGraphQuotient.evaluatedGraphQuotientEquivProductRoot`, `evaluatedGraphSpecProjection_finrank`, `EtaleGraphNeighborhood.exists_open_graph_isPullback`, `GeometricAssignedIncidenceNeighborhood.occurrenceAmbientIsoSupportAmbient`, `exists_occurrenceOpen_graphIdeal`, `exists_supportOpen_graphIdeal`, and their `PointedIncidenceDescent` consumers | checked: only points inserted from the assigned component product are used, so every specialized root sheet belongs to the genuine support member assigned to that occurrence; the selected sheet maps to its actual curve chart with exactly the corresponding affine-line coordinate and is the graph summand of the equal-coordinate pullback; the universal component root ring is formed over the ground-field section ring and specializes to global functions on the coherent affine support base, where every universal graph factor, the full multiplicity-sensitive graph product, and the graph-evaluation squares become the concrete owner-sheet equations. The quotient by this full evaluated product is canonically the product of the transported monic root algebras and therefore gives a finite-flat affine family of constant degree `d`. This coherent-base specialization is the checked algebra model but collapses repeated occurrence coordinates; the dimension-preserving affine occurrence chart above is the geometric comparison source. For each occurrence, the equal-coordinate locus is closed by separatedness and splits as the selected graph plus complementary étale sheets; deleting the closed image of the complement gives an open relative curve-chart neighbourhood in which the whole equal-coordinate locus is exactly the selected graph. The simultaneous intersection contains the exact tuple-sheet lift; the common base and all support pieces are simultaneously affine; and those pieces are assembled over the common base into one owner-compatible curve-family coproduct. What remains is to identify the restricted curve incidence ideal with the independently evaluated graph product |
| Affine ideal-sheaf base change | `AffineIdealSheafPullback.idealSheaf_comap_mul_of_isAffine`, `idealSheaf_comap_finsetProd_of_isAffine`, `EtaleGraphNeighborhood.equalCoordinateIdeal_comap_eq_graphToOpen_ker`, and `PointedIncidenceDescent.curveOrderedIncidenceIdeal_affineChart_comap_eq_prod_pullbackKernels` | checked: a pushout of affine section rings along the surjection defining a closed subscheme has kernel equal to extension of the original ideal, by tensor-product right exactness; consequently arbitrary affine base change preserves products of ideal sheaves. Thus, after the initial open restriction of the curve incidence ambient, the subsequent affine fpqc chart retains the full multiplicity-sensitive product even though its map is not an open immersion. On each isolating occurrence neighbourhood the equal-coordinate ideal pulls back exactly to the genuine graph ideal |
...
for the projective/Segre/Čech finiteness engine.  The genus definition below
uses the canonical arbitrary-field action, because the challenge quantifies
over every field and must not depend on rationalizing an abelian group.

The exact pinned Tau Ceti package contributes the bundled abelian-variety,
base-change, commutativity, and homomorphism-group vocabulary, but stops
before Jacobian construction and is kept on its separate Mathlib pin.  Lean
Pool contributes a checked complex-analytic Jacobian and a separate
Grothendieck-vanishing development.  The former cannot discharge an
arbitrary-field scheme theorem; the latter is a plausible input for the
eventual `H² = 0` deformation argument but does not represent the Picard
functor.  Other inspected relative-Picard developments provide useful tensor,
...
The quotient has now also been identified explicitly and its rank computed.
For a finite family of values `x_i` over a base ring, the product of graph
ideals is
...
where the left side denotes the product of principal ideals and the right
side the principal ideal generated by the product polynomial.
`SectionProductRootAlgebra.graphProductQuotientEquivRoot` therefore identifies
the quotient with the adjoining-root algebra of this monic polynomial.
`AffineLineSectionProduct` transports that calculation to global sections of
a relative affine line.  Finally,
`supportPieceGraphProductQuotientEquivRoot` transports it through the actual
geometric support-line isomorphism and the isolated étale support coordinate.
The quotient on one support member is finite free of rank
`Fintype.card (OccurrencesAtSupport ... a)`.  The checked fiber-cardinality
identity sums those ranks to `d`.

The calculation now lives on the actual affine support coproduct.  The
canonical finite-coproduct spectrum isomorphism induces
`supportCoproductToProductSectionsAlgEquiv`, and its component formula proves
that this algebra equivalence is literally restriction to each summand.
`SectionProductRootAlgebra.piIdealQuotientAlgEquiv` then identifies the
quotient of a finite product ring by the product of its component ideals with
the product of the component quotients.  Consequently
`supportCoproductGraphProductIdeal` is an ideal sheaf on the geometric
coproduct itself, and
`supportCoproductGraphProductQuotientEquivProduct` proves its quotient finite
flat of rank `d`, with
`orderedSupportGeometricAssigned_supportCoproductGraphProduct_finiteFlat_rank`
as the ordered-incidence consumer.  Moreover,
`graphToSupportCoproduct_ringHom_ker_eq_comap_pi` computes the kernel of each
actual occurrence graph in those product coordinates.  The componentwise
product lemma `pi_fintype_prod_single` then proves
...
ideal-identification and invariant-descent problem.  The morphism from the
support coproduct into the coordinate-base curve-level ordered ambient is
constructed, every owner occurrence graph factors through its matching
coordinate equalizer, and the checked cross-support affine refinement rules
out every off-owner equality locus scheme-theoretically while retaining all
owner sections.  The next step is to rebuild the support coproduct over this
refinement and prove that the pullback of each entire coordinate graph is
exactly its owner graph; the intrinsic product equality above will then
identify the full pulled-back incidence ideal.  Afterward compare the selected
block-invariant quotient with the block-monic coefficient chart.  Once that
comparison is cartesian, the existing pointwise fpqc interfaces descend the
isomorphism, flatness, rank, and Cartier equation to the universal effective
divisor.
...
neighbourhoods.  The checked affine occurrence chart now separates the
coordinate parameter base from the curve itself.  For every distinct
geometric support morphism it restricts the selected finite-étale curve
component over an affine base open, then places one independent copy of that
component in every assigned occurrence.  The resulting product has the
required `d` varying curve coordinates, carries the exact block action, maps
equivariantly and étale to ordered affine-root space, and maps equivariantly
back to the ordered curve power.  A canonical finite-power comparison
transports the geometrically deduplicated point across the ground-field base
isomorphism.  The local-ring product lifting theorem then produces a point of
the distinct selected-component product with that exact image, and
naturality under the assignment proves that its occurrence-wise repetition
maps to the original ordered divisor point while preserving the common
residue-field correlation.  Its finite block quotient and its stabilizer-
fixed central point are therefore checked at the actual divisor, not merely
at a tuple of underlying coordinate points.  The component product is now
also proved finite étale over the affine occurrence-base product; its
coordinate map factors through this morphism, and the correlated image of
the exact component point is block-fixed.  Equivariance descends that finite-
étale morphism to the two block quotients, where the coordinate-quotient map
factors through it.  This is the precise morphism to split on a pointwise
fpqc neighborhood.  Equivariance preserves its fiber-rank function, and the
fixed correlated base point now has a block-stable affine neighborhood
inside the constant-rank locus.  Its selected-component preimage is
block-stable as well, and an affine finite étale fpqc cover splits the
restricted map into exactly the original fiber rank of sheets.  The finite
relative product of all block translates of that cover is now constructed
and proved affine finite étale fpqc.  Its identity-indexed projection
refines the original splitting cover, while right translation of the product
indices gives self-maps over the block action and satisfies multiplication
after every projection.  The original split source and its slice
isomorphism are now pulled along the identity projection, producing the same
finite set of sheets over the common refinement and a canonical map back to
the selected-component preimage with its structural equation.  Pullback
equivariance then induces a genuine block action on the direct source, and
transport through the split slice isomorphism gives a genuine action on the
split finite source whose projection covers the action on the common
refinement.  The unique destination sheet is now extracted at every base
point and satisfies the translated-base cocycle law.  A fixed lift in the
refinement cannot be required: even an equivariant finite étale cover above a
fixed base point may have only nontrivial finite orbits.  Surjectivity instead
chooses an arbitrary lift of the exact correlated base point, equivariance
keeps its entire orbit over that point, and pullback-carrier surjectivity
selects an actual sheet above that lift mapping to the exact selected-
component point.  The same equivariance shows that every translated sheet
still maps to the fixed selected-component point.  Transition functions are
locally constant.  The union of the full transition-signature loci along the
finite orbit is a stable clopen neighborhood of that orbit; each individual
signature locus has constant sheet labels, and open-immersion factorization
upgrades those labels to exact scheme-morphism equations without requiring
the individual locus to be stable.  The next step is to compare this finite
orbitwise sheet representation with the prescribed blockwise model.  The
union of exactly the reached source sheets over their signature loci is now
constructed as a stable clopen open of the split source.  A clopen subscheme
of an affine scheme is proved affine by transporting it to the basic open of
an idempotent in the canonical spectrum.  The selected-sheet action therefore
has affine orbit neighborhoods and a finite-group quotient; its equivariant
map back to the original affine component chart has a clopen image.  That
image is stable, contains the exact selected-component point, and is affine
inside an affine component preimage.  Restricting the codomain to the image
turns the map into a surjective finite étale equivariant morphism, and the
resulting morphism descends canonically between the two local finite-group
quotients.  The target quotient is now also constructed directly as the
canonical affine local quotient `Spec Γ(U)ᴳ`.  Its quotient projection is
surjective, the composite from the selected sheet is invariant, and the
induced map from the selected-sheet quotient is proved surjective and to
satisfy the defining quotient square.  Thus the remaining comparison has a
literal invariant-ring target rather than only a glued quotient wrapper.
The ordered affine-coordinate target has now been identified with the
blockwise monic-coefficient chart.  The finite relative power of the affine
line is proved canonically isomorphic to the spectrum of the multivariable
root-coordinate ring, with exact coordinate projections and exact
equivariance for every permutation.  For the actual geometric-support tuple,
its stabilizer is conjugated to the product of the symmetric groups on the
tuple fibers.  Reindexing the variables along this conjugacy and applying the
fiberwise fundamental theorem of symmetric polynomials followed by Vieta's
signed reversal identifies its fixed ring with the polynomial ring of
blockwise monic coefficients.  Finally, the categorical universal properties
of Mazur's glued finite-group quotient and the explicit affine invariant
quotient give mutually inverse morphisms.  The resulting scheme isomorphism
commutes with the Vieta projection, and both the occurrence-base quotient and
the selected-component quotient now carry checked maps to this coefficient
space with their defining quotient squares.
...
ordered split-root family and proves finite-flat constant rank `d`.  The
per-occurrence neighbourhood is no longer missing:
`EtaleGraphNeighborhood` writes the equal-coordinate pullback as the chosen
graph plus a complement and deletes the complement's closed image, while
`GeometricAssignedIncidenceNeighborhood` applies that construction to every
actual support occurrence.  `AffineIdealSheafPullback` now proves that every
subsequent affine base change preserves the full product of graph ideals, and
the occurrence theorem records the exact equality of the pulled-back
equal-coordinate ideal with the genuine graph ideal.  The coherent root
coordinates are now proved to have one common structural composite to the
ground coordinate base.  Consequently each occurrence ambient is canonically
isomorphic to the support-indexed ambient, and transporting its isolating open
preserves both the cartesian graph square and the scheme-theoretic ideal
identity.  There is one moving curve variable, so the occurrence opens must
not be multiplied into artificial curve factors.  The occurrence-wise affine
chart now supplies the missing dimension-preserving version of this
construction: on every block-refinement tuple sheet, the selected curve
component at position `i` has exactly the independently varying `i`-th root
coordinate, and its graph-isolating open transports to the common affine
curve-family ambient of its support owner with the exact ideal equality
intact.  A finite union is useful only for containing all graphs; it does not
preserve every occurrence's ideal equality on the whole union.  The checked
simultaneous construction instead intersects the isolating opens and
restricts the refinement base to the locus where every graph lands in that
intersection.  Base changing the intersection to this common open makes
every selected occurrence graph a section.  Pullback pasting also proves on
the support intersection itself that each equal-coordinate ideal is exactly
the restricted selected graph ideal.  The common open is now proved to
contain the exact lifted divisor point.  The key argument is stronger than
equality of topological projections: the repeated assigned-product point has
equal residue-field morphisms in every occurrence of one support block.
After composing the tuple sheet with the residue-field point of its exact
lift, the corresponding graph morphisms into the support pullback are equal,
so they determine the same tensor-product prime.  Each graph value therefore
lies in every chosen isolating open for its support, and hence in their
intersection; finite-intersection membership gives the claimed point of
`simultaneousBaseOpen`.  Affine opens form a basis on each support ambient,
so one affine support piece is chosen around each common graph value inside
its isolating intersection.  After intersecting all graph preimages, a
second affine-basis choice on the refinement base gives a final affine open
through the exact lift on which every occurrence graph lands in its assigned
...
multiplicity-sensitive product.  Ordinary polynomial coordinates now carry
that product to `∏_i (X - x_i)`, so its quotient is a monic root algebra.
Transport through the common affine line and the isolated étale coordinate
proves that every support-piece quotient is finite free of its occurrence
multiplicity rank; their finite product is finite flat of total rank `d`.
The product-ring assembly has now also been identified intrinsically:
restriction to each coproduct summand computes the kernel of one occurrence
graph as the owner kernel on that summand and the unit ideal elsewhere, and
multiplication over all owners recovers exactly the assembled ideal sheaf.
The map from this coproduct to the curve-level ordered ambient over the
...
factorization property for invariant maps.  Applying it to the stable clopen
selected-component image gives a canonical map from `Spec Γ(U)ᴳ` to the
global block-monic coefficient space.  The selected orbit-sheet quotient
maps through this local quotient, and its composite agrees exactly with
including the actual selected sheet into the component chart and taking
blockwise monic coefficients.  What remains is to prove that this étale
slice is the required local pullback and then transport the ordered graph
ideal through that pullback.
...
MazurTorsion/AlgebraicGeometry/Jacobian/
  ...
  SectionProductRootAlgebra.lean
  AffineLineSectionProduct.lean
