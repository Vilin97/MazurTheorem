...
The blockwise invariant calculation now has its checked induction primitives.
...
of the full product of fiber permutation groups.  The remaining algebraic
step is to iterate these equivalences over the finitely many split sheets and
identify the resulting map with that joint coefficient map.  Combined with
`stabilizerEquivFiberPerm`, this will identify each actual quotient component
with `SplitComponentUniversalRoot`'s coefficient chart.
