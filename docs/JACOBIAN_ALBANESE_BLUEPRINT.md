...
| Split-component graph-product quotient | `SplitComponentGraphIdeal.orbitAdaptedPositionEquivRootIndex`, `rootGraphEvaluation_ker`, `orderedGraphIdeal_eq_orderedRootLocusIdeal`, `orderedGraphIdeal_eq_ker`, `SplitComponentGraphQuotient.orderedGraphQuotientEquivOrderedRoot`, `orderedRootLocusIsoGraphLocus_hom_comp_ι`, and `SplitComponentInvariantRootComparison.graphLocusIsoOrderedRootLocus_hom_comp_ι` | checked: an actual ordered tuple is reindexed by its sheet fibers using the permutation witnessing its relation to the component's arbitrary orbit representative; one root graph is exactly “select the owner sheet, then evaluate at the root,” with kernel the linear factor on that sheet and the unit ideal elsewhere; the product of all graph ideals is the exact kernel of simultaneous evaluation into the sheetwise ordered-root algebras, so repeated coordinates retain their scheme-theoretic factor multiplicity; the first isomorphism theorem now identifies the quotient by that actual graph-product ideal with the ordered split-root algebra as an algebra over the ground ring, and the induced scheme isomorphism commutes with both closed immersions into the common sheetwise affine-line ambient space |
...
the resulting isomorphism to the actual curve quotient.  Once that ideal is
transported, `SplitComponentGraphQuotient` supplies the checked
first-isomorphism-theorem bridge from its quotient algebra to the ordered
split-root family, compatible with the ambient closed immersions.  Only after
the geometric transport theorem may the pointwise fpqc interfaces be used to
