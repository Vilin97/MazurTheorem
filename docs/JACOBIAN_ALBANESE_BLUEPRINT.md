...
fiber product as that graph plus its complementary open subscheme.
...
component with the checked free-monic invariant chart, combine the now-checked
sheetwise graph decompositions over the finite coproduct, and prove that the
curve-level incidence restriction excludes all complementary graph factors.
