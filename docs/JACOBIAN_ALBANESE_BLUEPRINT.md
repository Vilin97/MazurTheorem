...
| Split-component ordered-incidence invariants | `SplitComponentOrderedIncidence.orderedIncidenceBaseChangeEquiv` and `invariantRootEquivOrderedIncidenceFixedPoints` | checked: tensor product commutes with the finite product of sheetwise root algebras, adjoining each root commutes with base change, and flat invariant base change recovers the product universal-root algebra from the ordered-root incidence invariants |
...
arbitrary characteristic.  What remains in this local layer is the geometric
comparison showing that the curve-level ordered incidence family, after
restriction to the chosen finite étale split chart, is this explicit product
algebra; its fpqc descent data must then be matched with the global pointed
comparison.
