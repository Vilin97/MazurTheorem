...
| Symmetric powers of the affine line | `AffineLineSymmetricPower.coefficientEquivFixedPoints` and `coordinateQuotientProjection_existsUnique_lift` | checked: permutation invariants are freely generated by elementary symmetric functions and give the categorical affine quotient |
...
coefficient space over an arbitrary base.  Mathlib's algebra classifying a
...
model through standard-smooth affine charts and their étale coordinates on a
smooth curve, identify it with the pointed symmetric-power/incidence morphism,
and descend the finite-flat Cartier property from that local comparison.
