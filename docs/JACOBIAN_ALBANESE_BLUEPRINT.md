...
canonically polynomial coefficient space.  The remaining algebraic transport
is to reindex the recursive list by the actual split sheets, identify its
product group with `stabilizerEquivFiberPerm`, and prove that the resulting
equivalence is the joint coefficient map used by
`SplitComponentUniversalRoot`.
