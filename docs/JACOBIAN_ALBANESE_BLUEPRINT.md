...
identity.  Repeated positions are still independent curve factors, so the
precise remaining step is to construct the finite relative product of these
transported occurrence opens over the coherent base, choose an affine
neighbourhood in that product, and identify its section-ring coordinate map
with `orderedAmbientCoordinateHom`.  Only after this geometric
