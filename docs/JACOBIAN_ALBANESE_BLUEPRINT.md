...
| Support-block monic root algebra | `SectionProductRootAlgebra.sectionPolynomial`, `graphProductQuotientEquivRoot`, and `graphProductQuotient_finrank`; `AffineLineSectionProduct.polynomialCoordinateAlgEquiv`, `map_sectionGraphProductIdeal`, and `sectionGraphProductQuotientEquivRoot`; `GeometricAssignedAffineSupportCoproduct.supportAffineLineGraphProductQuotientEquivRoot`, `supportPieceGraphProductQuotientEquivRoot`, `supportPieceGraphProductQuotient_finite`, `supportPieceGraphProductQuotient_flat`, and `supportGraphProductQuotientAlgebra_finrank`; `PointedIncidenceDescent.orderedSupportGeometricAssigned_supportGraphProduct_finiteFlat_rank` | checked: polynomial coordinates send the kernel of a section with value `x_i` to `(X - x_i)`, so the complete product quotient is the monic root algebra for `∏_i (X - x_i)`.  Transport through the standard relative-affine-line isomorphism and then through the isolated étale support coordinate identifies each geometric support-piece quotient with this algebra.  It is finite free of rank the number of ordered occurrences owned by that support member.  The fiber-cardinality identity sums these ranks to `d`, so the finite product over all support members is finite flat of rank exactly `d`; repeated sections contribute repeated polynomial factors and never lose nilpotent multiplicity. |
...
identity sums those ranks to `d`; hence
`supportGraphProductQuotientAlgebra` is finite flat of rank `d`, with
`orderedSupportGeometricAssigned_supportGraphProduct_finiteFlat_rank` as the
ordered-incidence consumer.

The remaining local comparison is no longer a rank calculation.  It is to
identify the quotient of the actual affine support coproduct (and then its
selected invariant quotient) with this checked finite product of
support-piece quotients, compatibly with the curve-level incidence closed
immersion and the block-monic coefficient chart.  Once that comparison is
