...
| Dimension-preserving affine root graphs | `GeometricAssignedAffineRootCoordinates.tupleSheetSection_comp_structure`, `tupleSheetToComponent_comp_base`, `tupleSheetToOccurrenceComponent_comp_coordinate`, `occurrenceCoordinate_comp_coordinateLine`, `occurrenceAmbientIsoSupportAmbient`, `exists_occurrenceOpen_graphIdeal`, `exists_supportOpen_graphIdeal`, `supportUnionOpen`, `graphToSupportUnion`, and `PointedIncidenceDescent.orderedSupportGeometricAssigned_tupleSheet_comp_coordinate` | checked: on the common block-refinement cover, every sheet of the split occurrence-component product is a section over the refinement base and projects to a genuine selected affine curve component at each occurrence. Its affine-line coordinate is exactly the corresponding coordinate of the dimension-`d` occurrence base, so repeated central support points remain independently deformable rather than being specialized to one value. Each independently varying graph has an isolating open on which the equal-coordinate ideal is exactly the graph ideal; because all occurrence coordinates have the same structural composite to the coordinate ground scheme, these opens transport to the single affine curve-family ambient indexed by their common support owner while preserving both the pullback square and the exact ideal equality. The simultaneous neighbourhood for one support owner is the finite union of those transported opens inside that one ambient, and every chosen graph factors through the union |
...
intact.  The finite union of these transported opens within each
support-indexed ambient is now constructed, with every chosen graph factored
through it.  The precise remaining step is to assemble those supportwise
unions in the common curve-family coproduct, choose the required affine
neighbourhoods there, and identify the resulting section-ring coordinate
