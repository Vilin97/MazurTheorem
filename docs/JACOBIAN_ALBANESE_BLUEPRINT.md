...
| Equivariant fpqc refinement | `EquivariantFpqcRefinement.refinement`, `refinement_isAffine_finiteEtale_fpqc`, `projection_one_comp_cover`, `rawActionHom_comp_cover`, and `rawActionHom_mul_comp_projection`; `GeometricAssignedAffineChart.componentFpqcBlockRefinement`; `PointedIncidenceDescent.orderedSupportGeometricAssigned_blockRefinement_isAffine_finiteEtale_fpqc` | checked: the finite relative product of every translate of an affine finite étale fpqc cover is again affine finite étale fpqc; its identity-indexed projection refines the original cover, its factor-translation maps cover the prescribed target action, and right translation of the product indices satisfies the multiplication law after every projection. These equations are the categorical input for transporting a split chart to a block-equivariant sheet presentation; an actual action on the transported split sheet labels remains to be constructed |
...
after every projection.  The next step is to pull the original split
isomorphism along the identity projection and lift these checked base maps
to a genuine block action on the transported sheet labels.  The quotient of
