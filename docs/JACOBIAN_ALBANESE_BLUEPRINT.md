...
| Finite étale split chart | `EtaleSplitChart.exists_fpqc_splitCover`, `AffineFiniteEtaleSplitChart.exists_fpqc_splitCover`, and `FiniteEtalePointSplitChart.exists_affineOpen_fpqc_splitCover` | checked: after a finite faithfully flat étale extension, a constant-rank finite étale algebra becomes a finite product of the base; the affine-scheme wrapper transports the resulting split pullback back through the canonical `isoSpec` squares, and every base point of a finite étale morphism has an affine constant-rank neighborhood carrying such a split fpqc cover |
...
| Fpqc finite-flat descent | `FpqcDescent` | checked: flatness and a constant finite-flat rank descend from an fpqc chart |
...
set after a finite étale fpqc cover.  The outstanding step is to combine
these charts for the finitely many support points of an ordered divisor, build
the induced neighborhood in the symmetric quotient, and exclude all
