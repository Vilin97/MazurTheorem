...
| Geometric block-monic coordinate quotient | `RelativeAffineLinePower.linePowerSchemeIsoPolynomial`, `AssignedTupleMonicCoordinates.monicCoefficientInvariantSchemeIso`, `GeometricBlockMonicChart.coordinateQuotientIsoMonicCoefficientSpace`, `quotientπ_comp_baseQuotientToMonicCoefficientSpace`, and `componentQuotientπ_comp_componentQuotientToMonicCoefficientSpace` | checked: the finite relative power of the affine line is the multivariable polynomial spectrum with exact coordinate projections and permutation equivariance; for an arbitrary support-assignment tuple its stabilizer is conjugated to the product of the symmetric groups of the assignment fibers, so its invariant ring is freely generated by the blockwise monic coefficients; categorical quotient uniqueness identifies Mazur's geometric ordered-coordinate quotient with this explicit coefficient space, and the actual affine-base and selected-component quotient maps satisfy the corresponding Vieta squares |
