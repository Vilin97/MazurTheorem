...
| Affine occurrence-wise block chart | `AssignedProductStabilizer.action`, `assignedMap_equivariant`, `productToAssignedProduct_comp_assignedMap`, `RelativePowerBaseIso.powerIso`, `powerPoint`, `assignmentRepeatHom_powerPoint`, `GeometricAssignedAffineChart.affineComponentFamily`, `commonAffineComponent`, `componentToCoordinatePower`, `componentToCurvePower`, `componentToCurvePower_commonAffineComponentPoint`, `componentQuotient`, and `PointedIncidenceDescent.orderedSupportGeometricAssignedAffinePoint_mapsToOrderedCurvePower` | checked: the affine coordinate base and the curve chart are kept distinct; each chosen finite-étale component is restricted over an affine base open through its support point, and one independent copy of that actual curve component is used for every ordered occurrence; the resulting degree-`d` product is affine and maps equivariantly and étale to ordered affine-root space; postcomposition along the ground-field base isomorphism canonically preserves finite powers and commutes with the occurrence assignment; the finite-product local-ring lifting theorem therefore constructs one correlated point in the product of distinct selected components mapping to the exact geometrically deduplicated curve tuple, and repeating it maps to the exact original ordered divisor point rather than merely agreeing coordinatewise; this central point is fixed by the block stabilizer, and the finite block quotient exists and is surjective |
...
at a tuple of underlying coordinate points.  After an equivariant fpqc
splitting, the selected sheets form one block-stable open, and quotienting
that open by
