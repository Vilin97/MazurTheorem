...
| Equivariant fpqc refinement | `EquivariantFpqcRefinement.refinement`, `refinement_isAffine_finiteEtale_fpqc`, `projection_one_comp_cover`, `rawActionHom_comp_cover`, `rawActionHom_mul`, and `refinementAction`; `EquivariantSplitRefinement.splitIso`, `directAction`, `splitAction`, and `splitAction_equivariant`; `GeometricAssignedAffineChart.componentFpqcBlockRefinement`, `componentFpqcBlockSplitIso`, and `componentFpqcBlockSplitAction`; `PointedIncidenceDescent.orderedSupportGeometricAssigned_blockRefinement_isAffine_finiteEtale_fpqc`, `orderedSupportGeometricAssigned_blockRefinementSplitIso`, and `orderedSupportGeometricAssigned_blockRefinementSplitAction` | checked: the finite relative product of every translate of an affine finite étale fpqc cover is again affine finite étale fpqc; its identity-indexed projection refines the original cover, and right translation of the product indices defines a genuine action covering the target action. Pulling an equivariant finite source and its split presentation to the common refinement induces an action on the direct pullback; transport through the slice isomorphism gives a genuine action on the split finite source whose projection is equivariant. Identifying these split-source automorphisms with the correct locally constant permutations of the sheet labels remains |
...
refinement.  The next step is to identify these automorphisms with locally
constant permutations of the sheet labels, refining by invariant clopens if
necessary.  The quotient of
