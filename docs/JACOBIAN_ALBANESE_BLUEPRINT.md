...
not be multiplied into artificial curve factors.  The precise remaining step
is instead to take their finite union within each support-indexed ambient,
assemble those supportwise opens in the common curve-family coproduct, choose
the required affine neighbourhoods there, and identify the resulting
section-ring coordinate maps with `orderedAmbientCoordinateHom`.  Only after this geometric
transport theorem may the pointwise fpqc interfaces be used to prove the
