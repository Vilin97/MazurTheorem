...
restricted map into exactly the original fiber rank of sheets.  The next
step is to replace that cover by a common refinement of its finitely many
block translates, so that the split sheet labels carry the descended block
action.  The quotient of the corresponding selected-sheet open by
