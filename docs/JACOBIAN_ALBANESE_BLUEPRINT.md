...
| Finite étale split chart | `EtaleSplitChart.exists_fpqc_splitCover` and `AffineFiniteEtaleSplitChart.exists_fpqc_splitCover` | checked: after a finite faithfully flat étale extension, a constant-rank finite étale algebra becomes a finite product of the base; the affine-scheme wrapper transports the resulting split pullback back through the canonical `isoSpec` squares to the original morphism |
...
pullback back through `isoSpec`.  The outstanding step is to combine the
finitely many pointwise components, build the induced neighborhood in the
symmetric quotient, and exclude all complementary sheets there.
