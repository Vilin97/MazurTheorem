...
now constructed, and every owner occurrence graph factors through its
matching coordinate equalizer.  It remains to identify each entire
pulled-back coordinate graph with that owner occurrence graph by ruling out
off-owner equality loci on a further neighborhood, then use the intrinsic
product equality above.  Afterward compare the selected
