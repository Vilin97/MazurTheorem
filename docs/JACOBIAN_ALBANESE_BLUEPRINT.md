...
from the coherent base.  This is the scheme-level input needed to recognize
the distinguished graph factor in each local incidence fiber product.
...
component with the checked free-monic invariant chart, apply the étale graph
coproduct theorem sheetwise using the new coordinate equation, and prove that
the curve-level incidence restriction excludes all complementary graph
factors.
