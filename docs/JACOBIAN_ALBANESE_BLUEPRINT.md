...
checked finite flat of constant rank `d`.  What remains in this local layer
is the geometric comparison showing that the curve-level ordered incidence
family, after restriction to the chosen finite étale split chart, is this
explicit product algebra; its fpqc descent data must then be matched with the
global pointed comparison.
