...
### 4.2 The rank-zero optimal quotient

Let `J₀(N)` be the Jacobian of `X₀(N)`. Embed the modular curve by the
Abel--Jacobi map based at `∞`, and project to an optimal quotient `A`.

For `N=11` or prime `N≥17`, take Mazur's Eisenstein quotient.  The retained
finite-flat descent proves `rank A(ℚ)=0`, and Mordell--Weil finite generation
makes `A(ℚ)` finite.  The stable interface is deliberately smaller than in
the old route:

* `A` is a nontrivial optimal quotient;
* `A(ℚ)` is finite; and
* the cusp-normalized map is a formal immersion at the auxiliary prime.

The exact order of `[0]-[∞]`, equality of `A(ℚ)` with the cuspidal subgroup,
and local principality of the Eisenstein ideal are not public consumers. They
may still be used internally if the quotient construction needs them.

At exact order `35`, use the optimal elliptic quotient `X₀(35)/w₅`, with
model
...
Its finite rational group is proved by a checked explicit descent.  This
fixed quotient then consumes the same formal-immersion API at auxiliary
prime eleven.

### 4.3 Formal immersion at the cusp

Let `A` be any nontrivial optimal quotient of the new part of `J₀(N)` and
let

\[
f:X_0(N)\longrightarrow J_0(N)\longrightarrow A
\]

be normalized by `f(∞)=0`. Mazur's 1978 Proposition 3.1 states that `f` is a
formal immersion along infinity away from characteristic two.

The cotangent argument is degree one. The cotangent space of `A` injects
into weight-two cusp forms and is Hecke-stable. After extending the residue
field, choose a nonzero simultaneous eigenvector. If its first q-expansion
coefficient `a₁` vanished, the standard Hecke recursions would force every
coefficient to vanish, contradicting the q-expansion principle. Thus the
induced cotangent map at infinity is nonzero; since the source curve has
one-dimensional cotangent space, it is surjective. This is the differential
criterion for formal immersion.

This is Mazur's own degree-one theorem. Kamienny's later symmetric-power
criterion needs linear independence of several Hecke operators; no such
higher-degree input belongs in this proof.

### 4.4 Potentially good reduction at five

Let `N=11` or let `N` be prime with `N≥17`, and suppose `P∈E(ℚ)` has exact
order `N`. The generated subgroup gives
`x=(E,⟨P⟩)∈X₀(N)(ℚ)`. Since the Eisenstein quotient has finite rational
points, `f(x)` has finite order.

Suppose `E` has potentially multiplicative reduction at five. Then `x`
specializes to a cusp. An Atkin--Lehner involution moves it to infinity, so
the sections `x` and `∞` meet in the special fibre. Their quotient images
meet at zero.

The specialization kernel of a Néron model over an unramified DVR contains
no nonzero torsion when `e<p-1`. Here `p=5` and `e=1`, so `f(x)=0`. The two
distinct generic sections now meet in the special fibre and have the same
image under `f`, which a formal immersion forbids: surjectivity onto the cusp
completion `ℤ₅[[q]]` distinguishes the section maps `q↦0` and `q↦q₀≠0`.
Hence `E` has potentially good reduction at five.

### 4.5 Good reduction and the ten-point contradiction

Since `N≠5`, the marked point retains exact order in the Néron special fibre.
That fibre cannot be additive. In residue characteristic five its component
group has order at most four, its nonsingular additive quotient has order
five, and its formal kernel is pro-five. None of these layers can contain a
point of prime order at least eleven. Potentially good reduction also
excludes a multiplicative fibre, so the reduction is good.

The marked point therefore injects into `E(𝔽₅)`, while Hasse gives

\[
\#E(\mathbb F_5)\le 5+1+2\sqrt5<11.
\]

This excludes order eleven and every prime order at least seventeen. Level
thirteen remains separate because `J₀(13)` is trivial.

The Lean acceptance proof does not need the general Hasse theorem. In
characteristic five every elliptic Weierstrass equation can be normalized to
short form `y²=x³+ax+b`; a decidable check of the 25 coefficient pairs proves
the same ten-point bound. This finite certificate is the selected
implementation, with Hasse retained as the mathematical explanation.

Mazur prints this finite-field finish at three. His Corollary 4.4 gives
potentially good reduction at every prime other than two, so five is a direct
specialization and is cleaner for Lean: it avoids wild characteristic-three
fibres while keeping the bound below eleven.

For exact order `35`, run the same squarefree-level collision at auxiliary
prime eleven. The level is prime to eleven, the local fibre arithmetic is
tame, and a corresponding check of the 121 short models gives
`#E(𝔽₁₁)≤18<35`. Order `25` cannot be absorbed because
`X₀(25)` has genus zero. Order `49` is cheaper still: the generated cyclic
subgroup maps directly to the already checked two-cusp classification of
`X₀(49)(ℚ)`.

The complete route comparison, interface audit, and fallbacks are in
[`ROUTE_AUDIT.md`](ROUTE_AUDIT.md).

## 5. Sources

The selected prime-order route is Barry Mazur,
[“Rational isogenies of prime degree”](https://www.math.columbia.edu/~goldfeld/Mazur-Goldfeld1978.pdf),
*Inventiones Mathematicae* **44** (1978), 129--162. Proposition 3.1 is the
formal-immersion theorem for any nontrivial optimal new quotient;
Corollaries 4.3--4.4 give the cusp collision and the finite Eisenstein
quotient; Theorem 4.1 finishes the torsion classification using a small
finite field.

The retained Eisenstein rank-zero construction comes from Barry Mazur,
...
The explicit level-35 quotient is recorded by Bryden Cais,
[“Universal Families and Ruling Out Small Primes”](https://math.stanford.edu/~conrad/vigregroup/vigre03/kubert.pdf),
VIGRE Mazur Seminar notes (2003), §4.  The notes construct
`X₀(35)/w₅ : z²+z=w³+w²+9w+1` and its rational torsion.  This roadmap
replaces their analytic rank assertion with a checked descent and their
fibre enumeration with the shared formal-immersion criterion at eleven.
...
For comparison, Sheldon Kamienny,
...
quotient to obtain uniform bounds. Those methods solve a higher-degree
tangent problem and add analytic rank-zero machinery; they are not required
by Mazur's degree-one Proposition 3.1.
