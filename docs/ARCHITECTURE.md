...
quotient with its kernel and base-change laws. The checked secant localization
is now the actual principal open in the affine scheme product and carries an
over-base addition morphism. The checked tangent localization is likewise an
actual principal open and carries an over-base projective doubling morphism.
These are prerequisites, not a group scheme. The group-scheme package remains
active until secant--tangent overlap compatibility, infinity charts, coverage,
gluing, multiplication laws, and the canonical `GrpObj` construction are
proved. The represented `Γ₀`-moduli package waits on the subgroup package.
Each step removes the corresponding supplied group-object, compatibility, or
arbitrary-interface input from its named order-49 consumer.
...
construction of the actual source and quotient Néron models remains open. No
Eisenstein instantiation is selected before those objects exist.
