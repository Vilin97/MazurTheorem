...
the second chart restricts to tangent doubling along the diagonal. These are
prerequisites, not a global group scheme. The package remains active until the
affine charts are glued, the infinity charts and coverage are added, and the
group axioms and coordinate-point comparison are proved. The represented
