...
- The audited baseline contains **2,019 Lean modules** and **2,092,446 Lean
  lines**.
...
| Represented `X₀(N)` vertical slice | `WP-MT-EC-ISOGENY-WEIL-EXACT-TORSION-SUBGROUP` | Construct the finite-flat cyclic subgroup generated by an exact-torsion point and refactor `MazurTorsion.XZeroFortyNine.rationalDatumOfSplitFiniteFlatSourceOfOrderFortyNineTorsion` to consume it without an arbitrary `M : WeierstrassGroupSchemeInterface E` input. | An exact-order-49 point reaches an honest represented `X₀(49)` point without a supplied point-equivalence shadow. |
...
- In the Weierstrass group-scheme package, the universal secant formula defines
  ...
  canonical Weierstrass group-scheme package is therefore integrated; the
  selected next package is the exact-torsion finite-flat cyclic subgroup.
