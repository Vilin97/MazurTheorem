...
- In coherent cohomology, multiplication by global functions now gives a
  ...
  unconditional. The raw full data now instantiate the divisor-indexed cocycle
  system expected by the Picard boundary; objectwise effectivity and descended
  tensor additivity for that concrete system are unconditional. The lane remains
  on coherent principal triviality, exact principal kernel, surjectivity, and the
...
- In the Weierstrass group-scheme package, the universal secant formula defines
  ...
  Associativity and coordinate-point compatibility are now checked. The named
  order-49 quotient consumer derives both from the canonical concrete cubic
  and no longer accepts supplied group-object or compatibility shadows. The
  canonical Weierstrass group-scheme package is therefore integrated. Every
