...
- `IsCoherentLowDegreeSupportComodel.of_isIso_restrict` and its proper-curve
  ...
  into the exact coherent residual-support interface;
...
- canonical field-linear `H⁰` functoriality, exact `H⁰`-`H⁰` and
  ...
  `H⁰(E)` automatic from a monomorphism `E ⟶ 𝒪_X^I`;
- `AffineOpenPresentation` and `exists_affineOpenPresentation`, which
  transpose finite generators on an affine open through the
  restriction--pushforward adjunction and pull back against the unit to build
  such an `E`, monic toward a finite free sheaf and epic toward the target on
  that open;
- `hZeroCanonical_finiteDimensional_of_affineOpenPresentation` and
  `hZeroCanonical_finiteDimensional_of_coherent`, whose kernel/image transfers
...
line bundle without a caller-supplied effectivity witness. The full raw datum
...
`ModuleDescentFullyFaithfulFor`, this lifts to the desired isomorphism of global
modules. A compatible family of adjoint local morphisms now lifts through the
